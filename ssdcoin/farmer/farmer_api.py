...
from ssdcoin.farmer.farmer import Farmer, increment_pool_stats, strip_old_entries
...
from ssdcoin.protocols.farmer_protocol import DeclareProofOfSpace, SignedValues
from ssdcoin.protocols.harvester_protocol import (
    PlotSyncDone,
    PlotSyncPathList,
    PlotSyncPlotList,
    PlotSyncStart,
...
from ssdcoin.server.server import ssl_context_for_root
...
from ssdcoin.types.blockchain_format.proof_of_space import (
...
from ssdcoin.types.stake_value import STAKE_PER_COEFFICIENT
...
class FarmerAPI:
    ...
    async def new_proof_of_space(
        self, new_proof_of_space: harvester_protocol.NewProofOfSpace, peer: WSSSDCoinConnection
    ) -> None:
        """
                ...
                try:
                    async with aiohttp.ClientSession() as session:
                        async with session.post(
                            ...
                            headers={"User-Agent": f"SSDCoin Blockchain v.{__version__}"},
                        ) as resp:
                            if not resp.ok:
    ...
    async def new_signage_point(
            self, new_signage_point: farmer_protocol.NewSignagePoint, peer: WSSSDCoinConnection
    ) -> None:
        ...
        try:
            ...
            response: farmer_protocol.FarmerStakeCoefficients = await peer.call_api(
                FullNodeAPI.request_stake_coefficients,
                farmer_protocol.RequestStakeCoefficients(
                    height=new_signage_point.peak_height,
                    farmer_public_keys=self.farmer.get_farmer_public_keys(),
                ),
            )
            if response is None:
                return
    ...
    @api_request(peer_required=True)
    async def farming_info(self, request: farmer_protocol.FarmingInfo, peer: WSSSDCoinConnection) -> None:
        self.farmer.state_changed(
            "new_farming_info",
            {
                "farming_info": {
    ...
    @api_request(peer_required=True)
    async def respond_plots(self, _: harvester_protocol.RespondPlots, peer: WSSSDCoinConnection) -> None:
        self.farmer.log.warning(f"Respond plots came too late from: {peer.get_peer_logging()}")

    @api_request(peer_required=True)
    async def plot_sync_start(self, message: PlotSyncStart, peer: WSSSDCoinConnection) -> None:
        await self.farmer.plot_sync_receivers[peer.peer_node_id].sync_started(message)

    @api_request(peer_required=True)
    async def plot_sync_loaded(self, message: PlotSyncPlotList, peer: WSSSDCoinConnection) -> None:
        await self.farmer.plot_sync_receivers[peer.peer_node_id].process_loaded(message)

    @api_request(peer_required=True)
    async def plot_sync_removed(self, message: PlotSyncPathList, peer: WSSSDCoinConnection) -> None:
        await self.farmer.plot_sync_receivers[peer.peer_node_id].process_removed(message)

    @api_request(peer_required=True)
    async def plot_sync_invalid(self, message: PlotSyncPathList, peer: WSSSDCoinConnection) -> None:
        await self.farmer.plot_sync_receivers[peer.peer_node_id].process_invalid(message)

    @api_request(peer_required=True)
    async def plot_sync_keys_missing(self, message: PlotSyncPathList, peer: WSSSDCoinConnection) -> None:
        await self.farmer.plot_sync_receivers[peer.peer_node_id].process_keys_missing(message)

    @api_request(peer_required=True)
    async def plot_sync_duplicates(self, message: PlotSyncPathList, peer: WSSSDCoinConnection) -> None:
        await self.farmer.plot_sync_receivers[peer.peer_node_id].process_duplicates(message)

    @api_request(peer_required=True)
    async def plot_sync_done(self, message: PlotSyncDone, peer: WSSSDCoinConnection) -> None:
    ...
    @api_request()
    async def respond_stake_coefficients(self, request: farmer_protocol.FarmerStakeCoefficients):
        self.farmer.log.warning("Respond stake coefficients too late")
