...
from ssdcoin.plot_sync.delta import Delta
from ssdcoin.plot_sync.receiver import Receiver
from ssdcoin.pools.pool_config import PoolWalletConfig, add_auth_key, load_pool_config
...
from ssdcoin.server.server import SSDCoinServer, ssl_context_for_root
...
from ssdcoin.util.config import config_path_for_filename, load_config, lock_and_load_config, save_config
from ssdcoin.util.errors import KeychainProxyConnectionFailure
...
from ssdcoin.util.logging import TimedDuplicateFilter
...
class Farmer:
    ...
    async def setup_keys(self) -> bool:
        no_keys_error_str = "No keys exist. Please run 'ssd keys generate' or open the UI."
        try:
        ...
        if "ssd_target_address" not in self.config:
            self.config = config["farmer"]
        if "ssd_target_address" not in self.pool_config:
            self.pool_config = config["pool"]
        if "ssd_target_address" not in self.config or "ssd_target_address" not in self.pool_config:
            log.debug("ssd_target_address missing in the config")
            return False

        # This is the farmer configuration
        self.farmer_target_encoded = self.config["ssd_target_address"]
        self.farmer_target = decode_puzzle_hash(self.farmer_target_encoded)

        self.pool_public_keys = [G1Element.from_bytes(bytes.fromhex(pk)) for pk in self.config["pool_public_keys"]]

        # This is the self pooling configuration, which is only used for original self-pooled plots
        self.pool_target_encoded = self.pool_config["ssd_target_address"]
    ...
    def set_reward_targets(self, farmer_target_encoded: Optional[str], pool_target_encoded: Optional[str]) -> None:
        with lock_and_load_config(self._root_path, "config.yaml") as config:
            if farmer_target_encoded is not None:
                ...
                config["farmer"]["ssd_target_address"] = farmer_target_encoded
            if pool_target_encoded is not None:
                self.pool_target_encoded = pool_target_encoded
                self.pool_target = decode_puzzle_hash(pool_target_encoded)
                config["pool"]["ssd_target_address"] = pool_target_encoded
