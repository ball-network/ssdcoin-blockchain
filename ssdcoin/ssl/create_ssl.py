...
from ssdcoin.util.ssl_check import DEFAULT_PERMISSIONS_CERT_FILE, DEFAULT_PERMISSIONS_KEY_FILE
...
def generate_ca_signed_cert(ca_crt: bytes, ca_key: bytes, cert_out: Path, key_out: Path):
    ...
    new_subject = x509.Name(
        [
            x509.NameAttribute(NameOID.COMMON_NAME, "SSDCoin"),
            x509.NameAttribute(NameOID.ORGANIZATION_NAME, "SSDCoin"),
    ...
    cert = (
        ...
        .add_extension(
            x509.SubjectAlternativeName([x509.DNSName("ssdcoin.top")]),
            critical=False,
        )
...
def make_ca_cert(cert_path: Path, key_path: Path):
    ...
    subject = issuer = x509.Name(
        [
            x509.NameAttribute(NameOID.ORGANIZATION_NAME, "SSDCoin"),
            x509.NameAttribute(NameOID.COMMON_NAME, "SSDCoin CA"),
            x509.NameAttribute(NameOID.ORGANIZATIONAL_UNIT_NAME, "Organic Farming Division"),
        ]
    )
    root_cert = (
...
def create_all_ssl(
    ...
    *,
    ...
    write_ssl_cert_and_key(ssdcoin_ca_crt_path, ssdcoin_ca_crt, ssdcoin_ca_key_path, ssdcoin_ca_key, overwrite=overwrite)
