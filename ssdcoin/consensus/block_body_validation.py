...
from ssdcoin.full_node.mempool_check_conditions import mempool_check_time_locks
...
from ssdcoin.types.stake_record import create_stake_farm_rewards
...
async def validate_block_body(
    ...
    *,
    ...
    if height > 0:
        ...
        if pool_reward > 0:
            expected_reward_coins.add(create_pool_coin(
        ...
        if prev_transaction_block_height >= STAKE_FORK_HEIGHT:
            ...
            if len(stake_records) > 0:
                expected_reward_coins.update(
        ...
        if prev_transaction_block.height > 0:
            ...
            while not curr_b.is_transaction_block:
                pool_reward = calculate_pool_reward(curr_b.height)
                if pool_reward > 0:
                    expected_reward_coins.add(
                        create_pool_coin(
                            curr_b.height,
                ...
                if curr_b.height >= STAKE_FORK_HEIGHT:
                    ...
                    if len(stake_records) > 0:
                        expected_reward_coins.update(
                            create_stake_farm_rewards(constants, stake_records, curr_b.height)
                        )
                else:
                    expected_reward_coins.add(
                        create_community_coin(
