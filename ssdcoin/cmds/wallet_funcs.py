...
from ssdcoin.cmds.cmds_util import (
    CMDTXConfigLoader,
    cli_confirm,
    get_wallet_client,
...
from ssdcoin.wallet.transaction_sorting import SortKey
from ssdcoin.wallet.util.address_type import AddressType, ensure_valid_address
...
from ssdcoin.wallet.wallet_coin_store import GetCoinRecords
...
def print_transaction(
    ...
    if verbose:
        ...
        ssdcoin_amount = Decimal(int(tx.amount)) / mojo_per_unit
        ...
        print(f"Amount {description}: {ssdcoin_amount} {name}")
...
async def send(
    *,
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        ...
        final_fee: uint64 = uint64(int(fee * units["ssdcoin"]))  # fees are always in SSD mojos
        ...
        print(f"To get status, use command: ssd wallet get_transaction -f {fingerprint} -tx 0x{tx_id}")
...
async def make_offer(
    *,
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        fee: int = int(d_fee * units["ssdcoin"])
        ...
        if offers == [] or requests == []:
            ...
            for item in [*offers, *requests]:
                            ...
                            if item in requests:
                                ...
                                if info.supports_did:
                                    ...
                                    royalty_asset_dict[name] = (
                                        encode_puzzle_hash(info.royalty_puzzle_hash, AddressType.SSD.hrp(config)),
                ...
                if fee > 0:
                    print()
                    print(f"Including Fees: {Decimal(fee) / units['ssdcoin']} SSD, {fee} mojos")
                ...
                if royalty_asset_dict != {}:
                    ...
                    for nft_id, summaries in royalty_summary.items():
                        ...
                        for summary in summaries:
                            divisor = units["ssdcoin"] if summary["asset"] == "SSD" else units["cat"]
                    ...
                    for asset, requested_amount in total_amounts_requested.items():
                        divisor = units["ssdcoin"] if asset == "SSD" else units["cat"]
                ...
                offer, trade_record = await wallet_client.create_offer_for_ids(
                    ...
                    tx_config=CMDTXConfigLoader(
                        reuse_puzhash=reuse_puzhash,
                    ).to_tx_config(units["ssdcoin"], config, fingerprint),
                )
                if offer is not None:
                    ...
                    print(
                        f"Use ssd wallet get_offers --id " f"{trade_record.trade_id} -f {fingerprint} to view status"
                    )
                else:
...
async def print_offer_summary(
    cat_name_resolver: CATNameResolver, sum_dict: Dict[str, int], has_fee: bool = False, network_ssd: str = "SSD"
) -> None:
    for asset_id, amount in sum_dict.items():
        description: str = ""
        unit: int = units["ssdcoin"]
        wid: str = "1" if asset_id == "ssd" else ""
        mojo_amount: int = int(Decimal(amount))
        name: str = network_ssd
        if asset_id != "ssd":
            name = asset_id
            if asset_id == "unknown":
                name = "Unknown"
...
async def print_trade_record(record: TradeRecord, wallet_client: WalletRpcClient, summaries: bool = False) -> None:
    ...
    if summaries:
        ...
        print(f"Included Fees: {fees / units['ssdcoin']} SSD, {fees} mojos")
    print("---------------")
...
async def take_offer(
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        ...
        offered, requested, _, _ = offer.summary()
        cat_name_resolver = wallet_client.cat_asset_id_to_name
        network_ssd = AddressType.SSD.hrp(config).upper()
        print("Summary:")
        print("  OFFERED:")
        await print_offer_summary(cat_name_resolver, offered, network_ssd=network_ssd)
        print("  REQUESTED:")
        await print_offer_summary(cat_name_resolver, requested, network_ssd=network_ssd)
        ...
        for royalty_asset_id in nft_coin_ids_supporting_royalties_from_offer(offer):
            if royalty_asset_id.hex() in offered:
                ...
                royalty_asset_dict[encode_puzzle_hash(royalty_asset_id, AddressType.NFT.hrp(config))] = (
                    encode_puzzle_hash(address, AddressType.SSD.hrp(config)),
                    percentage,
                )
        ...
        if royalty_asset_dict != {}:
            ...
            for fungible_asset_id in fungible_assets_from_offer(offer):
                fungible_asset_id_str = fungible_asset_id.hex() if fungible_asset_id is not None else "ssd"
                if fungible_asset_id_str in requested:
                    nft_royalty_currency: str = "Unknown CAT"
                    if fungible_asset_id is None:
                        nft_royalty_currency = network_ssd
                    else:
            ...
            if fungible_asset_dict != {}:
                ...
                for nft_id, summaries in royalty_summary.items():
                    print(f"  - For {nft_id}:")
                    for summary in summaries:
                        divisor = units["ssdcoin"] if summary["asset"] == network_ssd else units["cat"]
                        converted_amount = Decimal(summary["amount"]) / divisor
                        total_amounts_requested.setdefault(summary["asset"], fungible_asset_dict[summary["asset"]])
                        total_amounts_requested[summary["asset"]] += summary["amount"]
                        print(
                ...
                for asset, amount in total_amounts_requested.items():
                    divisor = units["ssdcoin"] if asset == network_ssd else units["cat"]
                    converted_amount = Decimal(amount) / divisor
                    print(f"  - {converted_amount} {asset} ({amount} mojos)")

        print(f"Included Fees: {Decimal(offer.fees()) / units['ssdcoin']} {network_ssd}, {offer.fees()} mojos")
        ...
        if not examine_only:
            ...
            trade_record = await wallet_client.take_offer(
                offer,
                fee=fee,
                tx_config=CMDTXConfigLoader().to_tx_config(units["ssdcoin"], config, fingerprint),
            )
            print(f"Accepted offer with ID {trade_record.trade_id}")
            print(f"Use ssd wallet get_offers --id {trade_record.trade_id} -f {fingerprint} to view its status")
...
async def cancel_offer(
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        ...
        await wallet_client.cancel_offer(
            offer_id, CMDTXConfigLoader().to_tx_config(units["ssdcoin"], config, fingerprint), secure=secure, fee=fee
        )
        print(f"Cancelled offer with ID {trade_record.trade_id}")
        if secure:
            print(f"Use ssd wallet get_offers --id {trade_record.trade_id} -f {fingerprint} to view cancel status")
...
def wallet_coin_unit(typ: WalletType, address_prefix: str) -> Tuple[str, int]:  # pragma: no cover
    ...
    if typ in [WalletType.STANDARD_WALLET, WalletType.POOLING_WALLET, WalletType.MULTI_SIG]:
        return address_prefix, units["ssdcoin"]
    return "", units["mojo"]
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        try:
            response = await wallet_client.did_message_spend(
                ...
                coin_announcements,
                CMDTXConfigLoader().to_tx_config(units["ssdcoin"], config, fingerprint),
            )
    ...
    *,
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        royalty_address = (
            ...
            else ensure_valid_address(royalty_address, allowed_types={AddressType.SSD}, config=config)
        )
        target_address = (
            None
            if not target_address
            else ensure_valid_address(target_address, allowed_types={AddressType.SSD}, config=config)
        )
        fee: int = int(d_fee * units["ssdcoin"])
        try:
...
async def transfer_nft(
    *,
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        try:
            target_address = ensure_valid_address(target_address, allowed_types={AddressType.SSD}, config=config)
            fee: int = int(d_fee * units["ssdcoin"])
...
async def send_notification(
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        ...
        amount: uint64 = uint64(d_amount * units["ssdcoin"])
        message: bytes = bytes(message_hex, "utf8")
        fee: uint64 = uint64(d_fee * units["ssdcoin"])

        tx = await wallet_client.send_notification(address, message, amount, fee)

        print("Notification sent successfully.")
        print(f"To get status, use command: ssd wallet get_transaction -f {fingerprint} -tx 0x{tx.name}")
...
async def sign_message(
    *,
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        if addr_type == AddressType.SSD:
            if address is None:
                print("Address is required for SSD address type.")
                return
    ...
    *, wallet_rpc_port: Optional[int], fp: Optional[int], fee: Decimal, tx_ids_str: str, force: bool = False
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, _, _):
        ...
        if fee < 0:
            ...
            return
        response = await wallet_client.spend_clawback_coins(tx_ids, int(fee * units["ssdcoin"]), force)
        print(str(response))
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        vc_record, txs = await wallet_client.vc_mint(
            ...
            if target_address is None
                ...
                ensure_valid_address(target_address, allowed_types={AddressType.SSD}, config=config)
            ),
            uint64(int(d_fee * units["ssdcoin"])),
        )
    ...
    *,
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        txs = await wallet_client.vc_spend(
            ...
            fee=uint64(int(d_fee * units["ssdcoin"])),
            tx_config=CMDTXConfigLoader(
                reuse_puzhash=reuse_puzhash,
            ).to_tx_config(units["ssdcoin"], config, fingerprint),
        )
...
async def approve_r_cats(
    ...
    async with get_wallet_client(wallet_rpc_port, fingerprint) as (wallet_client, fingerprint, config):
        ...
        txs = await wallet_client.crcat_approve_pending(
            ...
            fee=uint64(int(fee * units["ssdcoin"])),
            tx_config=CMDTXConfigLoader(
