...
from ssdcoin.cmds.cmds_util import transaction_status_msg, transaction_submitted_msg, get_wallet_client
from ssdcoin.cmds.units import units
from ssdcoin.rpc.wallet_rpc_client import WalletRpcClient
from ssdcoin.types.blockchain_format.sized_bytes import bytes32
from ssdcoin.types.stake_value import STAKE_FARM_LIST
...
from ssdcoin.wallet.util.query_filter import TransactionTypeFilter, HashFilter
...
def print_transaction(
    ...
    if verbose:
        ...
        ssdcoin_amount = Decimal(int(tx.amount)) / units["ssdcoin"]
        ...
        print(f"Amount {description}: {ssdcoin_amount} {address_prefix.upper()}")
...
async def stake_send(
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, _):
        ...
        final_amount = int(amount * units["ssdcoin"])
        if not (final_amount / units["ssdcoin"]).is_integer():
            print(
...
#             return
#         response = await wallet_client.spend_stake_coins(tx_ids, int(fee * units["ssdcoin"]))
#         print(str(response))
...
async def find_pool_nft(
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, _):
        ...
        total_amount = response["total_amount"] / units["ssdcoin"]
        record_amount = response["record_amount"] / units["ssdcoin"]
        balance_amount = response["balance_amount"] / units["ssdcoin"]
        print(f"Contract Address: {address}")
        print(f"Total Amount: {total_amount} SSD")
        print(f"Balance Amount: {balance_amount} SSD")
        print(f"Record Amount: {record_amount} SSD")
...
async def recover_pool_nft(
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, _):
        final_fee: uint64 = uint64(int(fee * units["ssdcoin"]))
        response = await wallet_client.recover_pool_nft(launcher_id, contract_address, final_fee)
        address = response["contract_address"]
        status = response["status"]
        amount = response["amount"] / units["ssdcoin"]
        print(f"Contract Address: {address}")
        print(f"Record Amount: {amount} SSD")
