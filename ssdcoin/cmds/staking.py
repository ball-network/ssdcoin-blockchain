...
@staking_cmd.command("send", short_help="Send ssdcoin to staking address")
@click.option(
    "-wp",
    "--wallet-rpc-port",
    help="Set the port where the Wallet is hosting the RPC interface. See the rpc_port under wallet in config.yaml",
...
@staking_cmd.command("withdraw", short_help="Withdraw staking ssdcoin")
@click.option(
    "-wp",
    "--wallet-rpc-port",
    help="Set the port where the Wallet is hosting the RPC interface. See the rpc_port under wallet in config.yaml",
...
@click.option(
    "-a",
    "--amount",
    help="withdraw staking SSD, 0 will withdraw all staking",
    type=str,
    default="0",
    show_default=True
)
