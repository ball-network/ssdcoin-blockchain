...
from ssdcoin.cmds.check_wallet_db import help_text as check_help_text
from ssdcoin.cmds.coins import coins_cmd
from ssdcoin.cmds.plotnft import validate_fee
...
@wallet_cmd.command("send", help="Send ssdcoin to another wallet")
@click.option(
    "-wp",
    "--wallet-rpc-port",
    help="Set the port where the Wallet is hosting the RPC interface. See the rpc_port under wallet in config.yaml",
...
@click.option("-f", "--fingerprint", help="Set the fingerprint to specify which key to use", type=int)
@click.option("-i", "--id", help="Id of the wallet to use", type=int, default=1, show_default=True, required=True)
@click.option("-a", "--amount", help="How much ssdcoin to send, in SSD", type=str, required=True)
@click.option("-e", "--memo", help="Additional memo for the transaction", type=str, default=None)
@click.option(
    "-m",
    "--fee",
    help="Set the fees for the transaction, in SSD",
    type=str,
    default="0",
    show_default=True,
    required=True,
)
@click.option("-t", "--address", help="Address to send the SSD", type=str, required=True)
@click.option(
...
@click.option(
    "-ma",
    "--min-coin-amount",
    help="Ignore coins worth less then this much SSD or CAT units",
    type=str,
    required=False,
    default="0",
)
@click.option(
    "-l",
    "--max-coin-amount",
    help="Ignore coins worth more then this much SSD or CAT units",
    type=str,
    required=False,
    default="0",
)
...
@click.option(
    "-m", "--fee", help="A fee to add to the offer when it gets taken, in SSD", default="0", show_default=True
)
def clawback(
...
@wallet_cmd.command("make_offer", help="Create an offer of SSD/CATs/NFTs for SSD/CATs/NFTs")
@click.option(
    "-wp",
    "--wallet-rpc-port",
    help="Set the port where the Wallet is hosting the RPC interface. See the rpc_port under wallet in config.yaml",
...
@click.option(
    "-m", "--fee", help="The fee to use when pushing the completed offer, in SSD", default="0", show_default=True
)
@click.option(
    "--reuse",
    help="Reuse existing address for the offer.",
...
@click.option(
    "-m", "--fee", help="The fee to use when cancelling the offer securely, in SSD", default="0", show_default=True
)
...
def check_wallet_cmd(ctx: click.Context, db_path: str, verbose: bool) -> None:
    """check, scan, diagnose, fsck SSDCoin Wallet DBs"""

    from ssdcoin.cmds.check_wallet_db import scan
...
@click.option(
    "-m",
    "--fee",
    help="Set the fees per transaction, in SSD.",
    type=str,
    default="0",
    show_default=True,
...
@click.option("-m", "--fee", help="The fee for the transaction, in SSD", type=str)
...
@click.option("-m", "--fee", help="Blockchain fee for mint transaction, in SSD", type=str, required=False, default="0")
def mint_vc_cmd(
    wallet_rpc_port: Optional[int],
    fingerprint: int,
    did: str,
...
@click.option(
    "-m", "--fee", help="Blockchain fee for update transaction, in SSD", type=str, required=False, default="0"
)
@click.option(
...
@click.option(
    "-m", "--fee", help="Blockchain fee for revocation transaction, in SSD", type=str, required=False, default="0"
)
@click.option(
    "--reuse-puzhash/--generate-new-puzhash",
    help="Send the VC back to the same puzzle hash it came from (ignored if --new-puzhash is specified)",
...
@wallet_cmd.command("find_pool_nft", short_help="fnd nft ssdcoin")
@click.option(
    "-wp",
    "--wallet-rpc-port",
    help="Set the port where the Wallet is hosting the RPC interface. See the rpc_port under wallet in config.yaml",
...
@click.option(
    "-l",
    "--launcher-id",
    help="Set the launcher id, you should get it from ssd wallet",
    type=str,
    required=True
)
@click.option(
...
@wallet_cmd.command("recover_pool_nft", short_help="recover nft ssdcoin to wallet")
@click.option(
    "-wp",
    "--wallet-rpc-port",
    help="Set the port where the Wallet is hosting the RPC interface. See the rpc_port under wallet in config.yaml",
