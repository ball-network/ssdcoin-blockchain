...
from ssdcoin.cmds.cmds_util import get_any_service_client
from ssdcoin.cmds.start_funcs import async_start
from ssdcoin.consensus.coinbase import create_puzzlehash_for_pk
from ssdcoin.simulator.simulator_full_node_rpc_client import SimulatorFullNodeRpcClient
...
def create_ssdcoin_directory(
    ...
    """
    This function creates a new ssdcoin directory and returns a heavily modified config,
    suitable for use in the simulator.
    """
    from ssdcoin.cmds.init_funcs import ssdcoin_init

    if not ssdcoin_root.is_dir() or not Path(ssdcoin_root / "config" / "config.yaml").exists():
        # create ssdcoin directories & load config
        ssdcoin_init(ssdcoin_root, testnet=True, fix_ssl_permissions=True)
        config: Dict[str, Any] = load_config(ssdcoin_root, "config.yaml")
        ...
        if not docker_mode:  # We want predictable ports for our docker image.
            ...
            config["network_overrides"]["config"]["simulator0"]["default_full_node_port"] = 31095 + port_offset
            # wallet
...
def display_key_info(fingerprint: int, prefix: str) -> None:
    """
    Display key info for a given fingerprint, similar to the output of `ssd keys show`.
    """
...
def generate_and_return_fingerprint(mnemonic: Optional[str] = None) -> int:
    ...
    """
    from ssdcoin.util.keychain import generate_mnemonic
...
async def generate_plots(config: Dict[str, Any], root_path: Path, fingerprint: int, bitfield: bool) -> None:
    ...
    from ssdcoin.simulator.start_simulator import PLOT_SIZE, PLOTS

    farming_puzzle_hash = decode_puzzle_hash(config["simulator"]["farming_address"])
    os.environ["SSDCOIN_ROOT"] = str(root_path)  # change env variable, to make it match what the daemon would set it to
...
async def async_config_wizard(
    ...
    if fingerprint is None:
        ...
        return
    # create ssdcoin directory & get config.
    print("Creating ssdcoin directory & config...")
    config = create_ssdcoin_directory(root_path, fingerprint, farming_address, plot_directory, auto_farm, docker_mode)
    ...
    sys.argv[0] = str(Path(sys.executable).parent / "ssdcoin")  # fix path for tests
    ...
    while True:
        try:
            ...
            break
    print(f"\nMake sure your SSDCOIN_ROOT Environment Variable is set to: {root_path}")
...
async def print_status(
    ...
    """
    from ssdcoin.cmds.show_funcs import print_blockchain_state
    ...
    async with get_any_service_client(SimulatorFullNodeRpcClient, rpc_port, root_path) as (node_client, config):
        ...
        # chain status ( basically ssd show -s)
        ...
        print(
            ...
            f"with a balance of: "
            f"{sum(coin_records.coin.amount for coin_records in farming_coin_records) / units['ssdcoin']} TSSD."
        )
        if show_addresses:
            print("All Addresses: ")
