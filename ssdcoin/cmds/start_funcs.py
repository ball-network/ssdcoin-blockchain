...
from ssdcoin.cmds.passphrase_funcs import get_current_passphrase
from ssdcoin.daemon.client import DaemonProxy, connect_to_daemon_and_validate
from ssdcoin.util.errors import KeychainMaxUnlockAttempts
from ssdcoin.util.keychain import Keychain
from ssdcoin.util.service_groups import services_for_groups
...
async def async_start(root_path: Path, config: Dict[str, Any], group: tuple[str, ...], restart: bool) -> None:
    ...
    if daemon is None:
        print("Failed to create the ssdcoin daemon")
        return None
