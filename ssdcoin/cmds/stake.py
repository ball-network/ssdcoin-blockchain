...
@stake_cmd.command("send", short_help="Send ssdcoin to stake")
@click.option(
    "-wp",
    "--wallet-rpc-port",
    help="Set the port where the Wallet is hosting the RPC interface. See the rpc_port under wallet in config.yaml",
...
@click.option("-f", "--fingerprint", help="Set the fingerprint to specify which wallet to use", type=int)
@click.option("-i", "--id", help="Id of the wallet to use", type=int, default=1, show_default=True, required=True)
@click.option("-a", "--amount", help="How much ssdcoin to stake, in SSD, must be a positive integer", type=int, required=True)
@click.option(
    "-m",
    "--fee",
    help="Set the fees for the stake transaction, in SSD",
    type=str,
    default="0",
    show_default=True,
    required=True,
...
@click.option(
    "-a",
    "--amount",
    help="withdraw stake SSD, 0 will withdraw all staked",
    type=str,
    default="0",
    show_default=True
)
