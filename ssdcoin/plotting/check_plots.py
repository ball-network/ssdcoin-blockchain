...
from ssdcoin.wallet.derive_keys import master_sk_to_farmer_sk, master_sk_to_local_sk
...
def check_plots(
    ...
    if len(plot_manager.no_key_filenames) > 0:
        log.warning(
            ...
            f"farm them, use 'ssd keys' to transfer keys. The pool public keys must be in the config.yaml"
        )
