...
from ssdcoin.plotting.util import parse_plot_info
from ssdcoin.types.blockchain_format.proof_of_space import generate_plot_public_key
...
class CacheEntry:
    ...
    def from_disk_prover(cls, prover: DiskProver) -> "CacheEntry":
        ...
        return cls(
            prover, farmer_public_key, pool_public_key, pool_contract_puzzle_hash, plot_public_key,
            local_public_key, time.time()
        )
        ...
        try:
            ...
            if stored_cache.version == CURRENT_VERSION:
                ...
                for path, cache_entry in cache_data.entries:
                    ...
                    #       Related: - https://github.com/ball-network/ssdcoin-blockchain/issues/13084
