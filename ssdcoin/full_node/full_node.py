...
from ssdcoin.full_node.sync_store import SyncStore
...
from ssdcoin.types.blockchain_format.vdf import CompressibleVDFField, VDFInfo, VDFProof
...
from ssdcoin.util.errors import ConsensusError, Err, ValidationError
...
class FullNode:
    ...
    root_path: Path
    config: Dict[str, Any]
    _server: Optional[SSDCoinServer]
    _shut_down: bool
    ...
    async def _sync(self) -> None:
        ...
        try:
            ...
            peers_with_peak: List[WSSSDCoinConnection] = [
    ...
    async def update_wallets(
        ...
        for peer, changes in changes_for_peer.items():
            if peer not in self.server.all_connections:
                continue
            ws_peer: WSSSDCoinConnection = self.server.all_connections[peer]
    ...
    async def add_unfinished_block(
        ...
        if await self.blockchain.check_staking_coefficient(block.reward_chain_block.proof_of_space):
            self.log.warning("unfinished_block staking coefficient error")
            return None
