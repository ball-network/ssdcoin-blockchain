...
class CoinStore:
    ...
    stake_height_cache: LRUCache[bytes48, uint128]
    ...
    async def create(cls, db_wrapper: DBWrapper2) -> CoinStore:
        ...
        self = CoinStore(db_wrapper, LRUCache(100), LRUCache(128))
    ...
    async def get_unspent_coins_before_height(self, puzzle_hash: bytes32, height: uint32) -> uint128:
        stake_key = bytes48(puzzle_hash + height.to_bytes(16, "big"))
        stake_amount: Optional[uint128] = self.stake_height_cache.get(stake_key)
        ...
        async with self.db_wrapper.reader_no_transaction() as conn:
            async with conn.execute(
                ...
                (puzzle_hash, height, height),
            ) as cursor:
                stake_amount = uint128(0)
                for row in await cursor.fetchall():
                    stake_amount += uint64.from_bytes(row[0])
                self.stake_height_cache.put(stake_key, stake_amount)
                return stake_amount
