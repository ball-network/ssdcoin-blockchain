...
from ssdcoin.types.blockchain_format.vdf import VDFInfo, validate_vdf
...
class FullNodeStore:
    ...
    def clear_candidate_blocks_below(self, height: uint32) -> None:
        ...
        for key, value in self.candidate_blocks.items():
            if value[0] < height or value[1].reward_chain_block.proof_of_space.stake_height < height:
                del_keys.append(key)
        for key in del_keys:
            try:
    ...
    def clear_unfinished_blocks_below(self, height: uint32) -> None:
        ...
        for partial_reward_hash, (unf_height, unfinished_block, _) in self.unfinished_blocks.items():
            if unf_height < height or unfinished_block.reward_chain_block.proof_of_space.stake_height < height:
