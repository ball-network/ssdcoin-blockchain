...
from ssdcoin.util.files import write_file_async
...
class BlockHeightMap:
    ...
    async def create(cls, blockchain_dir: Path, db: DBWrapper2) -> BlockHeightMap:
        ...
        async with self.db.reader_no_transaction() as conn:
            ...
            async with conn.execute(
                "SELECT header_hash,prev_hash,height,sub_epoch_summary "
                "FROM full_blocks WHERE header_hash=?",
                (peak_row[0],),
            ) as cursor:
    ...
    def update_height(
            self, height: uint32, header_hash: bytes32, ses: Optional[SubEpochSummary]
    ) -> None:
