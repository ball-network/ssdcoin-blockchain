...
from ssdcoin.types.stake_record import StakeRecord, StakeRecordThin
from ssdcoin.types.stake_value import STAKE_FARM_COUNT
...
class StakeRecordStore:
    ...
    async def create(cls, db_wrapper: DBWrapper2) -> StakeRecordStore:
        self = StakeRecordStore(db_wrapper, LRUCache(104))
    ...
    async def get_stake_amount_total(self, timestamp: uint64) -> Tuple[int, float]:
        async with self.db_wrapper.reader_no_transaction() as conn:
            async with conn.execute(
                ...
                stake_farm, stake_farm_calc = 0, 0.0
                for row in await cursor.fetchall():
                    if int(row[0]) == 1:
                        stake_farm = int(row[1])
                        stake_farm_calc = float(row[2])
        return stake_farm, stake_farm_calc
