...
from ssdcoin.protocols.protocol_state_machine import message_response_ok
from ssdcoin.protocols.protocol_timing import (
    API_EXCEPTION_BAN_SECONDS,
    CONSENSUS_ERROR_BAN_SECONDS,
    INTERNAL_PROTOCOL_ERROR_BAN_SECONDS,
)
from ssdcoin.protocols.shared_protocol import Capability, Error, Handshake
from ssdcoin.server.api_protocol import ApiProtocol
from ssdcoin.server.capabilities import known_active_capabilities
...
from ssdcoin.util.api_decorators import get_metadata
from ssdcoin.util.errors import ApiError, ConsensusError, Err, ProtocolError, TimestampError
from ssdcoin.util.ints import int16, uint8, uint16
...
ConnectionCallback = Callable[["WSSSDCoinConnection"], Awaitable[None]]
...
class ConnectionClosedCallbackProtocol(Protocol):
    def __call__(
        self,
        connection: WSSSDCoinConnection,
        ban_time: int,
...
class WSSSDCoinConnection:
    ...
    ssdcoin_full_version: Optional[Version] = None
    ...
    def create(
        ...
        return cls(
            ...
            session=session,
            ssdcoin_full_version=Version(ssdcoin_full_version_str()),
        )
    ...
    def is_old_version(self) -> bool:
        return Version(self.version) < self.ssdcoin_full_version
