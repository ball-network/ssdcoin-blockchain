...
from ssdcoin.protocols.protocol_state_machine import message_requires_reply
from ssdcoin.protocols.protocol_timing import INVALID_PROTOCOL_BAN_SECONDS
from ssdcoin.protocols.shared_protocol import protocol_version
from ssdcoin.server.api_protocol import ApiProtocol
from ssdcoin.server.introducer_peers import IntroducerPeers
from ssdcoin.server.outbound_message import Message, NodeType
from ssdcoin.server.ssl_context import private_ssl_paths, public_ssl_paths
from ssdcoin.server.ws_connection import ConnectionCallback, WSSSDCoinConnection
...
from ssdcoin.util.network import WebServer, is_in_network, is_localhost, is_trusted_peer
from ssdcoin.util.ssl_check import verify_ssl_certs_and_keys
...
class SSDCoinServer:
    ...
    all_connections: Dict[bytes32, WSSSDCoinConnection] = field(default_factory=dict)
    ...
    def create(
        ...
        ssdcoin_ca_crt_key: Tuple[Path, Path],
        name: str = __name__,
    ) -> SSDCoinServer:
        ...
        ssdcoin_ca_crt_path, ssdcoin_ca_key_path = ssdcoin_ca_crt_key
        ...
        if local_type in authenticated_client_types:
            ...
            ssl_client_context = ssl_context_for_client(
                ca_cert=ssdcoin_ca_crt_path,
                ca_key=ssdcoin_ca_key_path,
    ...
    async def garbage_collect_connections_task(self) -> None:
        ...
        while True:
            ...
            to_remove: List[WSSSDCoinConnection] = []
    ...
    async def start(
        ...
        if self.webserver is not None:
            raise RuntimeError("SSDCoinServer already started")
    ...
    async def incoming_connection(self, request: web.Request) -> web.StreamResponse:
        ...
        if peer_id == self.node_id:
            return ws
        connection: Optional[WSSSDCoinConnection] = None
        try:
            connection = WSSSDCoinConnection.create(
    ...
    async def connection_added(
        self, connection: WSSSDCoinConnection, on_connect: Optional[ConnectionCallback] = None
    ) -> None:
    ...
    def connection_closed(self, connection: WSSSDCoinConnection, ban_time: int, closed_connection: bool = False) -> None:
    ...
    async def send_to_others(
        ...
        node_type: NodeType,
        origin_peer: WSSSDCoinConnection,
    ) -> None:
    ...
    def is_trusted_peer(self, peer: WSSSDCoinConnection, trusted_peers: Dict[str, Any]) -> bool:
