...
from ssdcoin.daemon.server import service_launch_lock_path
from ssdcoin.rpc.rpc_server import RpcApiProtocol, RpcServer, RpcServiceProtocol, start_rpc_server
from ssdcoin.server.api_protocol import ApiProtocol
from ssdcoin.server.ssdcoin_policy import set_ssdcoin_policy
...
from ssdcoin.types.peer_info import PeerInfo, UnresolvedPeerInfo
...
from ssdcoin.util.network import resolve
...
class Service(Generic[_T_RpcServiceProtocol, _T_ApiProtocol]):
    def __init__(
        ...
        *,
        ...
        on_connect_callback: Optional[Callable[[WSSSDCoinConnection], Awaitable[None]]] = None,
        ...
        self._log.info(f"ssdcoin-blockchain version: {ssdcoin_full_version_str()}")
        ...
        self._server = SSDCoinServer.create(
            advertised_port,
            node,
            peer_api,
            node_type,
    ...
    async def setup_process_global_state(self, signal_handlers: SignalHandlers) -> None:
        ...
        proctitle_name = f"ssdcoin_{self._service_name}"
...
def async_run(coro: Coroutine[object, object, T], connection_limit: Optional[int] = None) -> T:
    with log_exceptions(log=log, message="fatal uncaught exception"):
        if connection_limit is not None:
            set_ssdcoin_policy(connection_limit)
