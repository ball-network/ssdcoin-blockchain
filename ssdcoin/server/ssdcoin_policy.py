...
if TYPE_CHECKING:
    ...
    class BaseEventsServer(asyncio.base_events.Server):
        if sys.platform == "win32":
            _loop: SSDCoinProactorEventLoop
        else:
...
class PausableServer(BaseEventsServer):
    ...
    def _detach(self) -> None:
        ...
        if (
            ...
            self._ssdcoin_resume()

    def _ssdcoin_pause(self) -> None:
...
async def _ssdcoin_create_server(
    cls: Any,
    protocol_factory: _ProtocolFactory,
    host: Any,
    port: Any,
...
class SSDCoinSelectorEventLoop(asyncio.SelectorEventLoop):
    ...
    async def create_server(self, *args, **kwargs) -> PausableServer:  # type: ignore[no-untyped-def]
        return await _ssdcoin_create_server(super(), *args, **kwargs)


if sys.platform == "win32":

    class SSDCoinProactor(IocpProactor):
        ...
        async def _ssdcoin_accept_loop(self, listener: socket.socket) -> Tuple[socket.socket, Tuple[object, ...]]:
            while True:
                ...
                try:
                    return await self._ssdcoin_accept(listener)
                except WindowsError as exc:  # pylint: disable=E0602
                    if exc.winerror not in (_winapi.ERROR_NETNAME_DELETED, _winapi.ERROR_OPERATION_ABORTED):
                        raise

        def _ssdcoin_accept(self, listener: socket.socket) -> asyncio.Future[Tuple[socket.socket, Tuple[object, ...]]]:
            ...
            async def accept_coro(self: SSDCoinProactor, future: asyncio.Future[object], conn: socket.socket) -> None:
        ...
        def accept(self, listener: socket.socket) -> asyncio.Future[Tuple[socket.socket, Tuple[object, ...]]]:
            coro = self._ssdcoin_accept_loop(listener)
            return asyncio.ensure_future(coro)

    class SSDCoinProactorEventLoop(ProactorEventLoop):
...
class SSDCoinPolicy(asyncio.DefaultEventLoopPolicy):
    def new_event_loop(self) -> asyncio.AbstractEventLoop:
        # overriding https://github.com/python/cpython/blob/v3.11.0/Lib/asyncio/events.py#L689-L695
        if sys.platform == "win32":
            loop_factory = SSDCoinProactorEventLoop
        else:
            loop_factory = SSDCoinSelectorEventLoop

        return loop_factory()


def set_ssdcoin_policy(connection_limit: int) -> None:
    ...
    asyncio.set_event_loop_policy(SSDCoinPolicy())
