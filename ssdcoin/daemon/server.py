...
from ssdcoin.cmds.init_funcs import check_keys, ssdcoin_full_version_str, ssdcoin_init
from ssdcoin.cmds.passphrase_funcs import default_passphrase, using_default_passphrase
from ssdcoin.consensus.coinbase import create_puzzlehash_for_pk
from ssdcoin.daemon.keychain_server import KeychainServer, keychain_commands
from ssdcoin.daemon.windows_signal import kill
from ssdcoin.plotters.plotters import get_available_plotters
from ssdcoin.plotting.util import add_plot_directory
from ssdcoin.server.server import ssl_context_for_server
from ssdcoin.util.bech32m import encode_puzzle_hash
from ssdcoin.util.beta_metrics import BetaMetricsLogger
from ssdcoin.util.ssdcoin_logging import initialize_service_logging
...
from ssdcoin.util.keychain import Keychain, KeyData, passphrase_requirements, supports_os_passphrase_storage
from ssdcoin.util.lock import Lockfile, LockfileError
from ssdcoin.util.log_exceptions import log_exceptions
from ssdcoin.util.misc import SignalHandlers
from ssdcoin.util.network import WebServer
...
service_plotter = "ssdcoin_plotter"
...
if getattr(sys, "frozen", False):
    name_map = {
        "ssdcoin_full_node_simulator": "ssdcoin_simulator",
    }

    def executable_for_service(service_name: str) -> str:
        executable = name_map[service_name] if service_name in name_map else service_name
        path = f"{os.path.dirname(sys.executable)}/{executable}"
...
class WebSocketServer:
        ...
        if self.ssl_context.minimum_version is not ssl.TLSVersion.TLSv1_3:
            self.log.warning(
                (
                    ...
                    "A future version of SSDCoin will require TLS1.3."
                ),
                ssl.OPENSSL_VERSION,
            )
    ...
    def _build_plotting_command_args(self, request: Any, ignoreCount: bool, index: int) -> List[str]:
        ...
        command_args: List[str] = ["ssdcoin", "plotters", plotter]

        if plotter == "bladebit":
            # plotter command must be either
            # 'ssd plotters bladebit ramplot' or 'ssd plotters bladebit diskplot'
            plot_type = request["plot_type"]
    ...
    async def exit(self) -> None:
        ...
        log.info("ssdcoin daemon exiting")
...
def launch_service(root_path: Path, service_command) -> Tuple[subprocess.Popen, Path]:
    ...
    log.debug(f"Launching service {service_array} with SSDCOIN_ROOT: {os.environ['SSDCOIN_ROOT']}")
...
async def async_run_daemon(root_path: Path, wait_for_unlock: bool = False) -> int:
    # When wait_for_unlock is true, we want to skip the check_keys() call in ssdcoin_init
    # since it might be necessary to wait for the GUI to unlock the keyring first.
    ssdcoin_init(root_path, should_check_keys=(not wait_for_unlock))
    ...
    try:
        with Lockfile.create(daemon_launch_lock_path(root_path), timeout=1):
            log.info(f"ssdcoin-blockchain version: {ssdcoin_full_version_str()}")
