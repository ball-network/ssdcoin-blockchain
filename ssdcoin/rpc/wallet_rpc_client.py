...
from ssdcoin.data_layer.data_layer_wallet import Mirror, SingletonRecord
...
from ssdcoin.wallet.conditions import Condition, ConditionValidTimes, conditions_to_json_dicts
...
from ssdcoin.wallet.util.query_filter import TransactionTypeFilter
...
from ssdcoin.wallet.vc_wallet.vc_store import VCRecord
...
class WalletRpcClient(RpcClient):
    ...
    async def nft_mint_bulk(
        ...
        ssd_coins: Optional[List[Dict]] = None,
        ssd_change_target: Optional[str] = None,
        ...
        request = {
            ...
            "ssd_coins": ssd_coins,
            "ssd_change_target": ssd_change_target,
    ...
    async def stake_info_old(self, wallet_id: int) -> Dict[str, Any]:
        response = await self.fetch("stake_info_old", {
            "wallet_id": wallet_id
        })
        return response

    async def stake_withdraw_old(
        self, wallet_id: int, amount: uint64,
    ) -> TransactionRecord:
        res = await self.fetch("stake_withdraw_old", {
            "wallet_id": wallet_id,
            "amount": amount,
        })
        return TransactionRecord.from_json_dict_convenience(res["transaction"])
