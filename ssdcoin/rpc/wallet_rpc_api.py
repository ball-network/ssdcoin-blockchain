...
from ssdcoin.consensus.block_rewards import calculate_base_farmer_reward, MOJO_PER_SSD
from ssdcoin.data_layer.data_layer_errors import LauncherCoinNotFoundError
from ssdcoin.data_layer.data_layer_wallet import DataLayerWallet
from ssdcoin.pools.pool_puzzles import create_p2_singleton_puzzle, POOL_OUTER_MOD_HASH
from ssdcoin.pools.pool_wallet import PoolWallet
from ssdcoin.pools.pool_wallet_info import FARMING_TO_POOL, PoolState, PoolWalletInfo, create_pool_state
...
from ssdcoin.rpc.rpc_server import Endpoint, EndpointResult, default_get_connections
from ssdcoin.rpc.util import tx_endpoint
...
from ssdcoin.types.blockchain_format.coin import Coin, coin_as_list
...
from ssdcoin.types.signing_mode import CHIP_0002_SIGN_MESSAGE_PREFIX, SigningMode
from ssdcoin.types.spend_bundle import SpendBundle, estimate_fees
from ssdcoin.types.stake_value import (
    STAKE_FARM_PREFIX,
    STAKE_FARM_LIST,
    STAKE_FARM_MIN,
    STAKE_FARM_COUNT,
...
from ssdcoin.util.config import load_config, str2bool
from ssdcoin.util.errors import KeychainIsLocked
from ssdcoin.util.ints import uint16, uint32, uint64, uint128
from ssdcoin.util.keychain import bytes_to_mnemonic, generate_mnemonic
from ssdcoin.util.misc import UInt32Range
...
from ssdcoin.wallet.cat_wallet.cat_info import CRCATInfo
from ssdcoin.wallet.cat_wallet.cat_wallet import CATWallet
from ssdcoin.wallet.cat_wallet.dao_cat_info import LockedCoinInfo
from ssdcoin.wallet.cat_wallet.dao_cat_wallet import DAOCATWallet
from ssdcoin.wallet.conditions import Condition
from ssdcoin.wallet.dao_wallet.dao_info import DAORules
...
from ssdcoin.wallet.did_wallet.did_info import DIDCoinData, DIDInfo
from ssdcoin.wallet.did_wallet.did_wallet import DIDWallet
from ssdcoin.wallet.did_wallet.did_wallet_puzzles import (
    DID_INNERPUZ_MOD,
    did_program_to_metadata,
    match_did_puzzle,
    metadata_to_program,
)
from ssdcoin.wallet.nft_wallet import nft_puzzles
from ssdcoin.wallet.nft_wallet.nft_info import NFTCoinInfo, NFTInfo
from ssdcoin.wallet.nft_wallet.nft_puzzles import get_metadata_and_phs
from ssdcoin.wallet.nft_wallet.nft_wallet import NFTWallet
from ssdcoin.wallet.nft_wallet.uncurry_nft import UncurriedNFT
from ssdcoin.wallet.notification_store import Notification
from ssdcoin.wallet.outer_puzzles import AssetType
from ssdcoin.wallet.payment import Payment
from ssdcoin.wallet.puzzle_drivers import PuzzleInfo, Solver
from ssdcoin.wallet.puzzles.clawback.metadata import AutoClaimSettings, ClawbackMetadata
from ssdcoin.wallet.puzzles.p2_delegated_puzzle_or_hidden_puzzle import puzzle_hash_for_synthetic_public_key
from ssdcoin.wallet.puzzles.stake.metadata import AutoWithdrawStakeSettings
from ssdcoin.wallet.singleton import (
    SINGLETON_LAUNCHER_PUZZLE_HASH,
    create_singleton_puzzle,
    get_inner_puzzle_from_singleton,
)
from ssdcoin.wallet.trade_record import TradeRecord
from ssdcoin.wallet.trading.offer import Offer
...
from ssdcoin.wallet.util.address_type import AddressType, is_valid_address
...
from ssdcoin.wallet.util.query_filter import HashFilter, TransactionTypeFilter
from ssdcoin.wallet.util.transaction_type import CLAWBACK_INCOMING_TRANSACTION_TYPES, TransactionType
from ssdcoin.wallet.util.tx_config import DEFAULT_TX_CONFIG, CoinSelectionConfig, CoinSelectionConfigLoader, TXConfig
from ssdcoin.wallet.util.wallet_sync_utils import fetch_coin_spend_for_coin_state
from ssdcoin.wallet.util.wallet_types import CoinType, WalletType
from ssdcoin.wallet.vc_wallet.cr_cat_drivers import ProofsChecker
from ssdcoin.wallet.vc_wallet.cr_cat_wallet import CRCATWallet
from ssdcoin.wallet.vc_wallet.vc_store import VCProofs
...
from ssdcoin.wallet.wallet_coin_store import CoinRecordOrder, GetCoinRecords, unspent_range
...
class WalletRpcApi:
    ...
    def __init__(self, wallet_node: WalletNode):
        ...
        self.service_name = "ssdcoin_wallet"
    ...
    def get_routes(self) -> Dict[str, Endpoint]:
        return {
            ...
            # Stake Old
            "/stake_info_old": self.stake_info_old,
            "/stake_withdraw_old": self.stake_withdraw_old,
            # Stake
    ...
    async def get_latest_singleton_coin_spend(
        self, peer: WSSSDCoinConnection, coin_id: bytes32, latest: bool = True
    ...
    async def _check_key_used_for_rewards(
        ...
        farmer_target = config["farmer"].get("ssd_target_address")
        pool_target = config["pool"].get("ssd_target_address")
    ...
    async def create_new_wallet(
        ...
        if request["wallet_type"] == "cat_wallet":
            ...
            if request["mode"] == "new":
                ...
                from ssdcoin.pools.pool_wallet_info import initial_pool_state_from_dict
    ...
    async def get_transactions(self, request: Dict[str, Any]) -> EndpointResult:
        ...
        for tr in transactions:
            try:
                ...
                if tx["type"] == TransactionType.INCOMING_STAKE_FARM_RECEIVE.value:
    ...
    async def verify_signature(self, request: Dict[str, Any]) -> EndpointResult:
        ...
        # signatures made by `ssd keys sign`, which uses BLS_MESSAGE_AUGMENTATION_HEX_INPUT
        ...
        if signing_mode == SigningMode.CHIP_0002:
            ...
            #   ("SSDCoin Signed Message", message)
    ...
    async def get_offer_summary(self, request: Dict[str, Any]) -> EndpointResult:
        ...
        from ssdcoin.util.bech32m import bech32_decode, convertbits
        from ssdcoin.wallet.util.puzzle_compression import OFFER_MOD_OLD, decompress_object_with_puzzles
    ...
    async def cancel_offers(
        ...
        if cancel_all:
            ...
            asset_id = request.get("asset_id", "ssd")
        ...
        key = None
        if asset_id is not None and asset_id != "ssd":
    ...
    async def did_get_info(self, request: Dict[str, Any]) -> EndpointResult:
        ...
        return {
            ...
            "p2_address": encode_puzzle_hash(p2_puzzle.get_tree_hash(), AddressType.SSD.hrp(self.service.config)),
    ...
    async def nft_mint_bulk(
        ...
        ssd_coin_list = request.get("ssd_coins", None)
        ssd_coins = None
        if ssd_coin_list:
            ssd_coins = {Coin.from_json_dict(ssd_coin) for ssd_coin in ssd_coin_list}
        ssd_change_target = request.get("ssd_change_target", None)
        if ssd_change_target is not None:
            if ssd_change_target[:2] == "ssd":
                ssd_change_ph = decode_puzzle_hash(ssd_change_target)
            else:
                ssd_change_ph = bytes32(hexstr_to_bytes(ssd_change_target))
        else:
            ssd_change_ph = None
        ...
        if mint_from_did:
            ...
            txs = await nft_wallet.mint_from_ssd(
                metadata_list,
                mint_number_start=mint_number_start,
                mint_total=mint_total,
                target_list=target_list,
    ...
    async def get_farmed_amount(self, request: Dict[str, Any]) -> EndpointResult:
        ...
        for record in tx_records:
            ...
            elif record.type == TransactionType.STAKE_FARM_REWARD.value:
                stake_farm_reward_amount += record.amount
        ...
        assert (
            amount == pool_reward_amount + farmer_reward_amount + stake_farm_reward_amount + fee_amount
        )
        return {
    ...
    async def get_auto_withdraw_stake(self, request) -> EndpointResult:
        auto_withdraw_stake_settings = AutoWithdrawStakeSettings.from_json_dict(
            self.service.wallet_state_manager.config.get("auto_withdraw_stake", {})
        )
    ...
    async def stake_info(self, request: Dict[str, Any]) -> EndpointResult:
        ...
        current_timestamp = self.service.wallet_state_manager.blockchain.get_latest_timestamp()
        stake_reward = await self.service.wallet_state_manager.tx_store.get_stake_rewards()
        balance = 0
        balance_exp = 0
        balance_other = 0
        balance_income = 0
        farm_puzzle_hash = self.service.wallet_state_manager.farm_puzzle_hash
        for tr in coin_records:
            coin: Coin = tr.additions[0]
            record: Optional[WalletCoinRecord] = await self.service.wallet_state_manager.coin_store.get_coin_record(
                coin.name()
            )
        ...
        return {
            ...
            "balance_income": uint128(balance_income),
            "balance_other": uint128(balance_other),
    ...
    async def stake_send(
        ...
        if get_stake_value(stake_type).time_lock == 0:
            return {
                "success": False,
                "error": f"stake type error {stake_type}",
            }
        if amount < STAKE_FARM_MIN:
            return {"success": False, "error": f"stake farm must more then {int(STAKE_FARM_MIN/MOJO_PER_SSD)} SSD"}
        if address != "":
        ...
        async with self.service.wallet_state_manager.lock:
            [tx] = await wallet.generate_signed_transaction(
                ...
                puzzle_decorator_override=[{
                    "decorator": PuzzleDecoratorType.STAKE.name,
                    "stake_type": stake_type,
                    "is_stake_farm": 1,
                    "recipient_ph": recipient_ph,
                }],
            )
            await wallet.push_transaction(tx)
        return {
    ...
    async def stake_withdraw_old(self, request: Dict[str, Any]) -> EndpointResult:
        if await self.service.wallet_state_manager.synced() is False:
            return {"success": False, "error": "Wallet needs to be fully synced before withdraw stake transactions"}
        if not isinstance(request["amount"], int):
            return {"success": False, "error": "An integer amount is required (too many decimals)"}

        wallet_id = uint32(request["wallet_id"])

        coin_records = await self.service.request_coin_records_by_puzzle_hash(
            self.service.wallet_state_manager.farm_puzzle_hash,
        )
        if len(coin_records) == 0:
            return {"success": False, "error": "No stake record"}
        ...
        for coin in sorted_coins:
            if amount == 0:
                ...
                if total_amount >= amount:
                    break
        if len(coins_rec) > max_num_coins:
            return {
                "success": False,
                "error": f"Old Withdraw transaction of {amount} mojo, {len(coins_rec)} coins would use more than "
                         f"{max_num_coins} coins. Try sending a smaller amount"
            }

        if total_amount == 0:
            return {"success": False, "error": f"withdraw amount min mojo of {sorted_coins[-1].amount}"}
        if amount > total_amount:
            return {"success": False, "error": f"Stake amount not more {total_amount}"}
        if total_amount > 0:
            wallet = self.service.wallet_state_manager.get_wallet(id=wallet_id, required_type=Wallet)
            puzzle_hash = await wallet.get_puzzle_hash(False)
            async with self.service.wallet_state_manager.lock:
                [tx] = await wallet.generate_signed_transaction(
                    uint64(total_amount),
                    puzzle_hash,
    ...
    async def stake_info_old(self, request: Dict[str, Any]) -> EndpointResult:
        coin_records = await self.service.request_coin_records_by_puzzle_hash(
            self.service.wallet_state_manager.farm_puzzle_hash,
        )
        selected = self.service.config["selected_network"]
        ...
        return {
            ...
            "address": encode_puzzle_hash(self.service.wallet_state_manager.farm_puzzle_hash, prefix),
        }
