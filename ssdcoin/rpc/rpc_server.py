...
from ssdcoin.rpc.util import wrap_http_handler
from ssdcoin.server.outbound_message import NodeType
from ssdcoin.server.server import SSDCoinServer, ssl_context_for_client, ssl_context_for_server
...
from ssdcoin.util.config import str2bool
from ssdcoin.util.ints import uint16
from ssdcoin.util.json_util import dict_to_json_str
from ssdcoin.util.network import WebServer, resolve
from ssdcoin.util.ws_message import WsRpcMessage, create_payload, create_payload_dict, format_response, pong
...
class RpcServiceProtocol(Protocol):
    ...
    def server(self) -> SSDCoinServer:
        ...
        # a property so as to be read only which allows SSDCoinServer to satisfy
        # Optional[SSDCoinServer]
        ...
    ...
    def get_connections(self, request_node_type: Optional[NodeType]) -> List[Dict[str, Any]]:
        ...
        ssdcoin.rpc.rpc_server.default_get_connections()
        """
        ...
...
def default_get_connections(server: SSDCoinServer, request_node_type: Optional[NodeType]) -> List[Dict[str, Any]]:
