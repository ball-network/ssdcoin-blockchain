...
from ssdcoin.consensus.block_rewards import (
    calculate_base_farmer_reward,
    calculate_pool_reward,
    calculate_stake_farm_reward,
)
from ssdcoin.consensus.coinbase import (
    farmer_parent_id,
    pool_parent_id,
    create_puzzlehash_for_pk,
    stake_farm_reward_parent_id,
)
from ssdcoin.consensus.constants import ConsensusConstants
from ssdcoin.data_layer.data_layer_wallet import DataLayerWallet
from ssdcoin.data_layer.dl_wallet_store import DataLayerStore
...
from ssdcoin.rpc.rpc_server import StateChangedProtocol
...
from ssdcoin.types.coin_spend import CoinSpend, compute_additions
...
from ssdcoin.types.stake_value import STAKE_FARM_MIN
from ssdcoin.util.bech32m import encode_puzzle_hash
from ssdcoin.util.db_synchronous import db_synchronous_on
from ssdcoin.util.db_wrapper import DBWrapper2
...
from ssdcoin.util.misc import UInt32Range, UInt64Range, VersionedBlob
from ssdcoin.util.path import path_from_root
from ssdcoin.util.streamable import Streamable
from ssdcoin.wallet.cat_wallet.cat_constants import DEFAULT_CATS
from ssdcoin.wallet.cat_wallet.cat_info import CATCoinData, CATInfo, CRCATInfo
from ssdcoin.wallet.cat_wallet.cat_utils import CAT_MOD, CAT_MOD_HASH, construct_cat_puzzle, match_cat_puzzle
from ssdcoin.wallet.cat_wallet.cat_wallet import CATWallet
from ssdcoin.wallet.cat_wallet.dao_cat_wallet import DAOCATWallet
from ssdcoin.wallet.conditions import Condition, ConditionValidTimes, parse_timelock_info
from ssdcoin.wallet.dao_wallet.dao_utils import (
...
from ssdcoin.wallet.dao_wallet.dao_wallet import DAOWallet
from ssdcoin.wallet.db_wallet.db_wallet_puzzles import MIRROR_PUZZLE_HASH
...
from ssdcoin.wallet.did_wallet.did_info import DIDCoinData
from ssdcoin.wallet.did_wallet.did_wallet import DIDWallet
from ssdcoin.wallet.did_wallet.did_wallet_puzzles import DID_INNERPUZ_MOD, match_did_puzzle
from ssdcoin.wallet.key_val_store import KeyValStore
from ssdcoin.wallet.nft_wallet.nft_puzzles import get_metadata_and_phs, get_new_owner_did
from ssdcoin.wallet.nft_wallet.nft_wallet import NFTWallet
from ssdcoin.wallet.nft_wallet.uncurry_nft import NFTCoinData, UncurriedNFT
from ssdcoin.wallet.notification_manager import NotificationManager
from ssdcoin.wallet.outer_puzzles import AssetType
from ssdcoin.wallet.payment import Payment
from ssdcoin.wallet.puzzle_drivers import PuzzleInfo
from ssdcoin.wallet.puzzles.clawback.drivers import generate_clawback_spend_bundle, match_clawback_puzzle
from ssdcoin.wallet.puzzles.clawback.metadata import ClawbackMetadata, ClawbackVersion
from ssdcoin.wallet.puzzles.p2_delegated_puzzle_or_hidden_puzzle import (
    DEFAULT_HIDDEN_PUZZLE_HASH,
    calculate_synthetic_secret_key,
    puzzle_hash_for_synthetic_public_key,
)
from ssdcoin.wallet.puzzles.stake.drivers import match_stake_puzzle, generate_stake_spend_bundle
from ssdcoin.wallet.puzzles.stake.metadata import StakeMetadata, StakeVersion
from ssdcoin.wallet.sign_coin_spends import sign_coin_spends
from ssdcoin.wallet.singleton import create_singleton_puzzle, get_inner_puzzle_from_singleton, get_singleton_id_from_puzzle
from ssdcoin.wallet.trade_manager import TradeManager
from ssdcoin.wallet.trading.trade_status import TradeStatus
from ssdcoin.wallet.transaction_record import TransactionRecord, TransactionRecordOld
from ssdcoin.wallet.uncurried_puzzle import uncurry_puzzle
from ssdcoin.wallet.util.address_type import AddressType
from ssdcoin.wallet.util.compute_hints import compute_spend_hints_and_additions
from ssdcoin.wallet.util.compute_memos import compute_memos
from ssdcoin.wallet.util.puzzle_decorator import PuzzleDecoratorManager
from ssdcoin.wallet.util.query_filter import HashFilter
from ssdcoin.wallet.util.transaction_type import CLAWBACK_INCOMING_TRANSACTION_TYPES, TransactionType
from ssdcoin.wallet.util.tx_config import TXConfig, TXConfigLoader
from ssdcoin.wallet.util.wallet_sync_utils import (
    PeerRequestException,
    fetch_coin_spend_for_coin_state,
    last_change_height_cs,
)
from ssdcoin.wallet.util.wallet_types import CoinType, WalletIdentifier, WalletType
from ssdcoin.wallet.vc_wallet.cr_cat_drivers import CRCAT, ProofsChecker, construct_pending_approval_state
from ssdcoin.wallet.vc_wallet.cr_cat_wallet import CRCATWallet
from ssdcoin.wallet.vc_wallet.vc_drivers import VerifiedCredential
from ssdcoin.wallet.vc_wallet.vc_store import VCStore
from ssdcoin.wallet.vc_wallet.vc_wallet import VCWallet
from ssdcoin.wallet.wallet import Wallet
from ssdcoin.wallet.wallet_blockchain import WalletBlockchain
from ssdcoin.wallet.wallet_coin_record import MetadataTypes, WalletCoinRecord
...
from ssdcoin.wallet.wallet_nft_store import WalletNftStore
from ssdcoin.wallet.wallet_pool_store import WalletPoolStore
from ssdcoin.wallet.wallet_protocol import WalletProtocol
from ssdcoin.wallet.wallet_puzzle_store import WalletPuzzleStore
from ssdcoin.wallet.wallet_retry_store import WalletRetryStore
...
class WalletStateManager:
    ...
    withdraw_coin_cache: Dict[bytes32, int] = {}
    ...
    stake_reward_puzzle_hash: bytes32
    ...
    async def create(
        ...
        self.farm_puzzle_hash = create_puzzlehash_for_pk(master_sk_to_farmer_sk(private_key).get_g1())
        self.stake_reward_puzzle_hash = create_puzzlehash_for_pk(
            master_sk_to_wallet_sk_unhardened(private_key, uint32(1)).get_g1()
        )
    ...
    async def get_private_key(self, puzzle_hash: bytes32) -> PrivateKey:
        # stake try farmer key
        if self.farm_puzzle_hash == puzzle_hash:
            return master_sk_to_farmer_sk(self.private_key)
    ...
    async def get_private_key_for_pubkey(self, pubkey: G1Element) -> Optional[PrivateKey]:
        ...
        if record is None:
            # stake try farmer key
            private = calculate_synthetic_secret_key(
                master_sk_to_farmer_sk(self.private_key), DEFAULT_HIDDEN_PUZZLE_HASH,
            )
            if private.get_g1() == pubkey:
                return private
            return None
        if record.hardened:
            return master_sk_to_wallet_sk(self.private_key, record.index)
    ...
    async def determine_coin_type(
        self, peer: WSSSDCoinConnection, coin_state: CoinState, fork_height: Optional[uint32]
        ...
        if coin_state.created_height is not None and (
            ...
            or self.is_stake_farm_reward(uint32(coin_state.created_height), coin_state.coin)
        ):
            return None, None
        ...
        # Check if the coin is staked
        if coin_state.coin.amount >= STAKE_FARM_MIN:
            stake_coin_data = match_stake_puzzle(uncurried, puzzle, solution)
            if stake_coin_data is not None:
                return await self.handle_stake(stake_coin_data, coin_state, coin_spend, peer), stake_coin_data
    ...
    async def spend_clawback_coins(
        ...
        for coin, metadata in clawback_coins.items():
            try:
                ...
                # Remove the clawback hint since it is unnecessary for the SSD coin
        ...
        if fee > 0:
            ssdcoin_tx = await self.main_wallet.create_tandem_ssd_tx(
                fee, tx_config, Announcement(coin_spends[0].coin.name(), message)
            )
            assert ssdcoin_tx.spend_bundle is not None
            spend_bundle = SpendBundle.aggregate([spend_bundle, ssdcoin_tx.spend_bundle])
        assert derivation_record is not None
    ...
    async def filter_spam(self, new_coin_state: List[CoinState]) -> List[CoinState]:
        ssd_spam_amount = self.config.get("ssd_spam_amount", 1000000)

        # No need to filter anything if the filter is set to 1 or 0 mojos
        if ssd_spam_amount <= 1:
            return new_coin_state

        spam_filter_after_n_txs = self.config.get("spam_filter_after_n_txs", 200)
        small_unspent_count = await self.coin_store.count_small_unspent(ssd_spam_amount)
        ...
        for cs in new_coin_state:
            ...
            if (
                ...
                and cs.coin.amount < ssd_spam_amount
    ...
    async def get_minter_did(self, launcher_coin: Coin, peer: WSSSDCoinConnection) -> Optional[bytes32]:
        # Get minter DID
                ...
                async with self.db_wrapper.writer():
                    ...
                    # if the coin has been spent
                        ...
                        if record is None:
                            farmer_reward = False
                            pool_reward = False
                            stake_farm_reward = False
                            created_height = uint32(coin_state.created_height)
                            tx_type: int
                            if self.is_farmer_reward(created_height, coin_state.coin):
                                farmer_reward = True
                                tx_type = TransactionType.FEE_REWARD.value
                            elif self.is_pool_reward(created_height, coin_state.coin):
                                pool_reward = True
                                tx_type = TransactionType.COINBASE_REWARD.value
                            elif self.is_stake_farm_reward(created_height, coin_state.coin):
                                stake_farm_reward = True
                                tx_type = TransactionType.STAKE_FARM_REWARD.value
                            else:
                            ...
                            record = WalletCoinRecord(
                                ...
                                True,
                                farmer_reward or stake_farm_reward or pool_reward,
                                wallet_identifier.type,
                                wallet_identifier.id,
                            )
                            ...
                            if len(children) > 0:
                                ...
                                stake_metadata: Optional[StakeMetadata] = None
                                ...
                                for coin in additions:
                                    ...
                                    if derivation_record is None:  # not change
                                        ...
                                        if coin_spend is None:
                                            ...
                                            uncurried = uncurry_puzzle(puzzle)
                                            stake_metadata = match_stake_puzzle(uncurried, puzzle, solution)
                                            if stake_metadata is None:
                                                clawback_metadata = match_clawback_puzzle(uncurried, puzzle, solution)
                                            else:
                                                to_puzzle_hash = stake_metadata.stake_puzzle_hash
                                                if stake_metadata.is_stake_farm:
                                                    tx_type = TransactionType.OUTGOING_STAKE_FARM.value
                                        if stake_metadata is not None or clawback_metadata is not None:
    ...
    async def coin_added(
        ...
        elif self.is_stake_farm_reward(height, coin):
            tx_type = TransactionType.STAKE_FARM_REWARD
        else:
        ...
        coinbase = tx_type in {
            ...
            TransactionType.STAKE_FARM_REWARD,
        }
        ...
        clawback = parent_coin_record is not None and (
            parent_coin_record.coin_type == CoinType.CLAWBACK or parent_coin_record.coin_type == CoinType.STAKE
        )
    ...
    def is_stake_farm_reward(self, created_height: uint32, coin: Coin) -> bool:
        if coin.amount > calculate_stake_farm_reward(created_height):
            # Optimization to avoid the computation below. Any coin less than this base amount cannot be farmer reward
            return False
        for i in range(0, 30):
            try_height = created_height - i
            if try_height < 0:
                break
            calculated = stake_farm_reward_parent_id(uint32(try_height), self.constants.GENESIS_CHALLENGE)
            if calculated == coin.parent_coin_info:
                return True
        return False

    async def get_all_transactions_by_confirmed(
            self, wallet_id: int, record_type: int = None, confirmed: bool = False
    ) -> List[TransactionRecordOld]:
        """
        Retrieves all confirmed and pending transactions
        """
        records = await self.tx_store.get_all_transactions_for_wallet_by_confirmed(wallet_id, record_type, confirmed)
        return records

    async def handle_stake(
        self,
        ...
        """
        Handle Stake coins
        :param metadata: Stake metadata for spending the merkle coin
        :param coin_state: Stake merkle coin
        :param coin_spend: Parent coin spend
        :param peer: Fullnode peer
        :return:
        """
        ...
        assert coin_state.created_height is not None
        is_recipient: Optional[bool] = None
        # Check if the wallet is the stake
        coin_name = coin_state.coin.name()
        if await self.puzzle_store.get_derivation_record_for_puzzle_hash(
            metadata.recipient_puzzle_hash
        ) is not None:
            self.log.info("Found Stake merkle coin %s as the recipient.", coin_name.hex())
            is_recipient = True
            # For the recipient we need to manually subscribe the merkle coin
            await self.add_interested_coin_ids([coin_name])
        elif self.farm_puzzle_hash == metadata.stake_puzzle_hash:
            self.log.info("Found Stake merkle coin %s as the stake.", coin_name.hex())
            is_recipient = False
        ...
        if is_recipient is not None:
            ...
            spent_height: uint32 = uint32(0)
            if coin_state.spent_height is not None:
                self.log.debug("Resync stake coin: %s", coin_name.hex())
                if is_recipient is False:
                    await self.coin_store.delete_coin_record(coin_name)
                    await self.tx_store.delete_transaction_record(coin_name)
                    return None
                # Resync case
                spent_height = uint32(coin_state.spent_height)
                # Create Stake outgoing transaction
                # created_timestamp = await self.wallet_node.get_timestamp_for_height(uint32(coin_state.spent_height))
                # stake_coin_spend: CoinSpend = await fetch_coin_spend_for_coin_state(coin_state, peer)
                # stake_spend_bundle: SpendBundle = SpendBundle([stake_coin_spend], G2Element())
                # if await self.puzzle_store.puzzle_hash_exists(stake_spend_bundle.additions()[0].puzzle_hash):
                #     tx_record = TransactionRecord(
                #         confirmed_at_height=uint32(coin_state.spent_height),
                #         created_at_time=created_timestamp,
                #         to_puzzle_hash=metadata.recipient_puzzle_hash,
                ...
                #         confirmed=True,
                #         sent=uint32(0),
                #         spend_bundle=stake_spend_bundle,
                #         additions=stake_spend_bundle.additions(),
                #         removals=stake_spend_bundle.removals(),
                #         wallet_id=uint32(1),
                #         sent_to=[],
                #         trade_id=None,
                #         type=uint32(TransactionType.STAKE_WITHDRAW),
                #         name=stake_spend_bundle.name(),
                #         memos=list(compute_memos(stake_spend_bundle).items()),
                #         valid_times=ConditionValidTimes(),
                #     )
                #     await self.tx_store.add_transaction_record(tx_record)
            ...
            coin_record = WalletCoinRecord(
                ...
                1,
                CoinType.STAKE,
                VersionedBlob(StakeVersion.V1.value, bytes(metadata)),
            )
            # Add merkle coin
            await self.coin_store.add_coin_record(coin_record)
            # Add tx record
            # We use TransactionRecord.confirmed to indicate if a Stake transaction is Withdraw
            # If the Stake coin is unspent, confirmed should be false
            created_timestamp = await self.wallet_node.get_timestamp_for_height(uint32(coin_state.created_height))
            tx_record = TransactionRecord(
                confirmed_at_height=uint32(coin_state.created_height),
                created_at_time=uint64(created_timestamp),
                to_puzzle_hash=metadata.stake_puzzle_hash,
                amount=uint64(coin_state.coin.amount),
                fee_amount=uint64(0),
                confirmed=spent_height != 0,
                sent=uint32(0),
    ...
    async def auto_withdraw_stake_coins(self) -> None:
        # Get unspent stake coin
        current_timestamp = self.blockchain.get_latest_timestamp()
        stake_farm_coins: Dict[Coin, StakeMetadata] = {}
        tx_fee = uint64(self.config.get("auto_withdraw_stake", {}).get("tx_fee", 0))
        assert self.wallet_node.logged_in_fingerprint is not None
        tx_config_loader: TXConfigLoader = TXConfigLoader.from_json_dict(self.config.get("auto_withdraw_stake", {}))
        tx_config: TXConfig = tx_config_loader.autofill(
            constants=self.constants,
            config=self.config,
            logged_in_fingerprint=self.wallet_node.logged_in_fingerprint,
        ...
        batch_size = self.config.get("auto_withdraw_stake", {}).get("batch_size", 50)
        for coin in unspent_coins.records:
            try:
                metadata: MetadataTypes = coin.parsed_metadata()
                ...
                coin_timestamp = await self.wallet_node.get_timestamp_for_height(coin.confirmed_block_height)
                if current_timestamp - coin_timestamp >= metadata.time_lock:
                    if metadata.is_stake_farm:
                        if await self.puzzle_store.puzzle_hash_exists(metadata.recipient_puzzle_hash) is False:
                            await self.coin_store.delete_coin_record(coin.coin.name())
                            await self.tx_store.delete_transaction_record(coin.coin.name())
                            continue
                        stake_farm_coins[coin.coin] = metadata
                        if len(stake_farm_coins) >= batch_size:
                            await self.spend_stake_coins(stake_farm_coins, True, tx_fee, tx_config)
                            stake_farm_coins = {}
            except Exception as e:
                self.log.error(f"Failed to withdraw stake coin {coin.coin.name().hex()}: %s", e)
        if len(stake_farm_coins) > 0:
            await self.spend_stake_coins(stake_farm_coins, True, tx_fee, tx_config)

    async def spend_stake_coins(
        self,
        stake_coins: Dict[Coin, StakeMetadata],
        is_stake_farm: bool,
        fee: uint64,
        tx_config: TXConfig,
        force: bool = False,
        ...
        derivation_record: Optional[DerivationRecord] = None
        amount: uint64 = uint64(0)
        for coin, metadata in stake_coins.items():
            coin_name = coin.name()
            try:
                self.log.info(f"Withdraw stake coin {coin_name.hex()}")
                # Get incoming tx
                spend_count = self.withdraw_coin_cache.get(coin_name, 0) + 1
                incoming_tx = await self.tx_store.get_transaction_record(coin_name)
                assert incoming_tx is not None, f"Cannot find incoming tx for stake coin {coin_name.hex()}"
                if incoming_tx.sent > 0 and not force:
                    self.log.error(
                        f"Stake coin {coin_name.hex()} is already in a pending spend bundle. {incoming_tx}"
                    )
                    if spend_count < 2:
                        self.withdraw_coin_cache[coin_name] = spend_count
                        continue
                    else:
                        self.withdraw_coin_cache.pop(coin_name)
                ...
                coin_spend: CoinSpend = generate_stake_spend_bundle(coin, metadata, inner_puzzle, inner_solution)
                coin_spends.append(coin_spend)
            except Exception as e:
                self.log.error(f"Failed to create stake spend bundle for {coin_name.hex()}: {e}")
        if len(coin_spends) == 0:
            return []
        spend_bundle: SpendBundle = await self.sign_transaction(coin_spends)
        if fee > 0:
            ssd_tx = await self.main_wallet.create_tandem_ssd_tx(
                fee, tx_config, Announcement(coin_spends[0].coin.name(), message)
            )
            assert ssd_tx.spend_bundle is not None
            spend_bundle = SpendBundle.aggregate([spend_bundle, ssd_tx.spend_bundle])
        assert derivation_record is not None
        tx_record = TransactionRecord(
            confirmed_at_height=uint32(0),
            created_at_time=now,
