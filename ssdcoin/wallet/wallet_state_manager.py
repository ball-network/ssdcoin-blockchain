...
from ssdcoin.consensus.coinbase import farmer_parent_id, pool_parent_id, create_puzzlehash_for_pk
...
from ssdcoin.protocols.wallet_protocol import CoinState, NewPeakWallet
...
from ssdcoin.wallet.derive_keys import (
    ...
    master_sk_to_wallet_sk_unhardened_intermediate, master_sk_to_farmer_sk,
)
...
class WalletStateManager:
    ...
    async def get_private_key(self, puzzle_hash: bytes32) -> PrivateKey:
        ...
        if record is None:
            # staking try farmer key
            private = master_sk_to_farmer_sk(self.private_key)
            if create_puzzlehash_for_pk(private.get_g1()) == puzzle_hash:
                return private
