...
from ssdcoin.wallet.conditions import ConditionValidTimes
from ssdcoin.wallet.transaction_record import TransactionRecord, TransactionRecordOld, minimum_send_attempts
from ssdcoin.wallet.transaction_sorting import SortKey
from ssdcoin.wallet.util.query_filter import FilterMode, TransactionTypeFilter
...
class WalletTransactionStore:
    ...
    async def get_farming_rewards(self) -> List[TransactionRecord]:
        ...
        async with self.db_wrapper.reader_no_transaction() as conn:
            ...
            rows = await conn.execute_fetchall(
                ...
                (fee_int, pool_int, stake_farm_int),
            )
    ...
    async def get_unconfirmed_for_wallet(self, wallet_id: int) -> List[TransactionRecord]:
        ...
        async with self.db_wrapper.reader_no_transaction() as conn:
            rows = await conn.execute_fetchall(
                ...
                "AND type<>?",
                (wallet_id, TransactionType.INCOMING_STAKE_FARM_RECEIVE.value),
            )
        return await self._get_new_tx_records_from_old([TransactionRecordOld.from_bytes(row[0]) for row in rows])
    ...
    async def get_all_transactions_for_wallet_by_confirmed(
        ...
        if record_type is not None:
            type_str = f" AND type={record_type}"
        confirmed_str = ""
        if confirmed:
            confirmed_str = f" AND confirmed=0"
    ...
    async def get_stake_rewards(self) -> int:
        """
        Returns the all stake rewards.
        """
        async with self.db_wrapper.reader_no_transaction() as conn:
            rows = await conn.execute_fetchall(
                "SELECT amount from transaction_record WHERE confirmed=1 and type=?",
                (
                 TransactionType.STAKE_FARM_REWARD.value,
                ),
            )
            return sum(int(uint64.from_bytes(row[0])) for row in rows)
