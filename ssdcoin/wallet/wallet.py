...
from ssdcoin.wallet.puzzles.clawback.metadata import ClawbackMetadata
...
from ssdcoin.wallet.puzzles.puzzle_utils import (
...
from ssdcoin.wallet.util.wallet_types import WalletIdentifier, WalletType
...
class Wallet:
    ...
    async def create_tandem_ssd_tx(
        ...
        ssdcoin_coins = await self.select_coins(fee, tx_config.coin_selection_config)
        [ssdcoin_tx] = await self.generate_signed_transaction(
            uint64(0),
    ...
    # WSSSDCoinConnection is only imported for type checking
