...
from ssdcoin.protocols.wallet_protocol import CoinState, RequestBlockHeader, RespondBlockHeader
...
from ssdcoin.wallet import singleton
from ssdcoin.wallet.cat_wallet.cat_utils import CAT_MOD, SpendableCAT, construct_cat_puzzle
from ssdcoin.wallet.cat_wallet.cat_utils import get_innerpuzzle_from_puzzle as get_innerpuzzle_from_cat_puzzle
from ssdcoin.wallet.cat_wallet.cat_utils import unsigned_spend_bundle_for_spendable_cats
...
from ssdcoin.wallet.conditions import Condition, parse_timelock_info
from ssdcoin.wallet.dao_wallet.dao_info import DAOInfo, DAORules, ProposalInfo, ProposalType
...
from ssdcoin.wallet.util.tx_config import DEFAULT_TX_CONFIG, CoinSelectionConfig, TXConfig
...
class DAOWallet:
    ...
    It is not a user-facing wallet. A user cannot spend or receive SSD though a wallet of this type.
    ...
    # if asset_id == None: then we get normal SSD
    ...
    async def coin_added(self, coin: Coin, height: uint32, peer: WSSSDCoinConnection, coin_data: Optional[Any]) -> None:
        """
        ...
        try:
            ...
            if matched_funding_puz:
                # funding coin
                ssd_funds_puzhash = get_p2_singleton_puzhash(self.dao_info.treasury_id, asset_id=None)
                if coin.puzzle_hash == ssd_funds_puzhash:
                    asset_id = None
                else:
    ...
    async def resync_treasury_state(self) -> None:
        ...
        wallet_node: Any = self.wallet_state_manager.wallet_node
        peer: WSSSDCoinConnection = wallet_node.get_full_node_peer()
        if peer is None:  # pragma: no cover
            raise ValueError("Could not find any peers to request puzzle and solution from")
        ...
        parent_inner_puz = ssdcoin.wallet.singleton.get_inner_puzzle_from_singleton(
        ...
        # get existing ssd funds for treasury
        ssd_funds_puzhash = get_p2_singleton_puzhash(self.dao_info.treasury_id, asset_id=None)
        await self.wallet_state_manager.add_interested_puzzle_hashes([ssd_funds_puzhash], [self.id()])
    ...
    async def generate_proposal_vote_spend(
        ...
        if fee > 0:
            ...
            spend_bundle = unsigned_spend_bundle.aggregate([unsigned_spend_bundle, dao_cat_spend, ssdcoin_tx.spend_bundle])
        else:
    ...
    async def create_proposal_close_spend(
        ...
        if proposal_state["passed"] and not self_destruct:
            ...
            if proposal_type == ProposalType.SPEND:
                ...
                if sum > 0:
                    ssd_coins = await self.select_coins_for_asset_type(uint64(sum))
                    for ssd_coin in ssd_coins:
                        ssd_parent_amount_list.append([ssd_coin.parent_coin_info, ssd_coin.amount])
                        solution = Program.to(
                            [
                                0,
                                treasury_inner_puzhash,
                        ...
                        coin_spends.append(CoinSpend(ssd_coin, p2_singleton_puzzle, solution))
        ...
        if fee > 0:
            ssdcoin_tx = await self.standard_wallet.create_tandem_ssd_tx(fee, tx_config)
            assert ssdcoin_tx.spend_bundle is not None
            full_spend = SpendBundle.aggregate([spend_bundle, ssdcoin_tx.spend_bundle])
        else:
    ...
    async def free_coins_from_finished_proposals(
        ...
        if fee > 0:
            ssdcoin_tx = await self.standard_wallet.create_tandem_ssd_tx(fee, tx_config)
            assert ssdcoin_tx.spend_bundle is not None
            full_spend = full_spend.aggregate([full_spend, ssdcoin_tx.spend_bundle])
            ...
            if prop_info.proposal_id == proposal_id:
                ...
                if proposal_type == ProposalType.SPEND:
                    ...
                    dictionary: Dict[str, Any] = {
                        ...
                        "ssd_conditions": ssd_created_coins,
