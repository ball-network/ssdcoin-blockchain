...
from ssdcoin.wallet.vc_wallet.vc_store import VCRecord, VCStore
...
class VCWallet:
    ...
    async def revoke_vc(
        self, parent_id: bytes32, peer: WSSSDCoinConnection, fee: uint64 = uint64(0), reuse_puzhash: Optional[bool] = None
    ) -> List[TransactionRecord]:
        ...
        if fee > 0:
            ...
            tx = dataclasses.replace(tx, spend_bundle=SpendBundle.aggregate([ssdcoin_tx.spend_bundle, tx.spend_bundle]))
            ssdcoin_tx = dataclasses.replace(ssdcoin_tx, spend_bundle=None)
            return [tx, ssdcoin_tx]
        else:
