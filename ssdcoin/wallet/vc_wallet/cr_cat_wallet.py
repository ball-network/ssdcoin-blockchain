...
from ssdcoin.util.misc import VersionedBlob
from ssdcoin.wallet.cat_wallet.cat_info import CATCoinData, CRCATInfo
from ssdcoin.wallet.cat_wallet.cat_utils import CAT_MOD_HASH, CAT_MOD_HASH_HASH, construct_cat_puzzle
...
from ssdcoin.wallet.util.tx_config import TXConfig
...
from ssdcoin.wallet.vc_wallet.cr_cat_drivers import (
    CRCAT,
    CRCATMetadata,
    CRCATVersion,
    ProofsChecker,
...
class CRCATWallet(CATWallet):
    ...
    async def coin_added(
        self, coin: Coin, height: uint32, peer: WSSSDCoinConnection, coin_data: Optional[CATCoinData]
    ) -> None:
    ...
    async def _generate_unsigned_spendbundle(
            ...
            if first:
                ...
                if need_ssdcoin_transaction:
                    if fee > regular_ssdcoin_to_claim:
                        ...
                        ssdcoin_tx, ssd_announcement = await self.create_tandem_ssd_tx(
                            fee,
                            uint64(regular_ssdcoin_to_claim),
                            tx_config,
                        )
                        assert ssd_announcement is not None
                        innersol = self.standard_wallet.make_solution(
                            primaries=primaries,
                            coin_announcements={announcement.message},
                            coin_announcements_to_assert={ssd_announcement.name()},
                            conditions=extra_conditions,
                        )
                else:
                    innersol = self.standard_wallet.make_solution(
        ...
        return (
            SpendBundle(
                [
                    ...
                    *(
                        (
                            spend
                            for bundle in [ssdcoin_tx.spend_bundle]
            ...
            [*vc_txs, *([ssdcoin_tx] if ssdcoin_tx is not None else [])],
        )
    ...
    async def claim_pending_approval_balance(
        ...
        nonce: bytes32 = Program.to(
            [coin_as_list(c) for c in sorted(coins.union(ssdcoin_coins).union({vc.coin}), key=Coin.name)]
        ).get_tree_hash()
        ...
        if fee > 0:
            ...
            if ssdcoin_tx.spend_bundle is None:
                raise RuntimeError("Did not get spendbundle for fee transaction")  # pragma: no cover
            claim_bundle = SpendBundle.aggregate([claim_bundle, ssdcoin_tx.spend_bundle])
        else:
            ssdcoin_tx = None
        ...
        other_txs: List[TransactionRecord] = [
            ...
            *((dataclasses.replace(ssdcoin_tx, spend_bundle=None),) if ssdcoin_tx is not None else []),
        ]
