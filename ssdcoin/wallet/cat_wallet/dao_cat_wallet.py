...
from ssdcoin.wallet.cat_wallet.dao_cat_info import DAOCATInfo, LockedCoinInfo
...
from ssdcoin.wallet.util.curry_and_treehash import calculate_hash_of_quoted_mod_hash
...
class DAOCATWallet:
    ...
    async def remove_active_proposal(
        ...
        if fee > 0:  # pragma: no cover
            ssdcoin_tx = await self.standard_wallet.create_tandem_ssd_tx(fee, tx_config=tx_config)
            assert ssdcoin_tx.spend_bundle is not None
            full_spend = SpendBundle.aggregate([spend_bundle, ssdcoin_tx.spend_bundle])
        else:
            full_spend = spend_bundle
