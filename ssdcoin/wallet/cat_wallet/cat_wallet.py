...
from ssdcoin.types.coin_spend import compute_additions_with_cost
...
from ssdcoin.wallet.cat_wallet.cat_info import CATCoinData, CATInfo, LegacyCATInfo
from ssdcoin.wallet.cat_wallet.cat_utils import (
    CAT_MOD,
    SpendableCAT,
...
from ssdcoin.wallet.cat_wallet.lineage_store import CATLineageStore
from ssdcoin.wallet.coin_selection import select_coins
from ssdcoin.wallet.conditions import Condition, ConditionValidTimes, UnknownCondition, parse_timelock_info
...
from ssdcoin.wallet.puzzles.tails import ALL_LIMITATIONS_PROGRAMS
...
from ssdcoin.wallet.util.curry_and_treehash import calculate_hash_of_quoted_mod_hash, curry_and_treehash
...
class CATWallet:
    ...
    async def create_new_cat_wallet(
        ...
        try:
            ssdcoin_tx, spend_bundle = await ALL_LIMITATIONS_PROGRAMS[
        ...
        ssdcoin_tx = dataclasses.replace(ssdcoin_tx, spend_bundle=spend_bundle, name=spend_bundle.name())
        await self.standard_wallet.push_transaction(ssdcoin_tx)
    ...
    async def coin_added(
        self, coin: Coin, height: uint32, peer: WSSSDCoinConnection, parent_coin_data: Optional[CATCoinData]
    ) -> None:
    ...
    async def create_tandem_ssd_tx(
        self,
        fee: uint64,
        ...
        if fee > amount_to_claim:
            ssdcoin_coins = await self.standard_wallet.select_coins(
                fee,
                tx_config.coin_selection_config,
            )
            origin_id = list(ssdcoin_coins)[0].name()
            [ssdcoin_tx] = await self.standard_wallet.generate_signed_transaction(
                uint64(0),
            ...
            origin_id = list(ssdcoin_coins)[0].name()
            selected_amount = sum([c.amount for c in ssdcoin_coins])
            [ssdcoin_tx] = await self.standard_wallet.generate_signed_transaction(
            ...
            message = None
            for spend in ssdcoin_tx.spend_bundle.coin_spends:
        ...
        return ssdcoin_tx, announcement
    ...
    async def generate_unsigned_spendbundle(
        ...
        # Figure out if we need to absorb/melt some SSD as part of this
        regular_ssdcoin_to_claim: int = 0
        if payment_amount > starting_amount:
            fee = uint64(fee + payment_amount - starting_amount)
        elif payment_amount < starting_amount:
            regular_ssdcoin_to_claim = payment_amount

        need_ssdcoin_transaction = (fee > 0 or regular_ssdcoin_to_claim > 0) and (fee - regular_ssdcoin_to_claim != 0)
        ...
        for coin in cat_coins:
            ...
            if first:
                ...
                if need_ssdcoin_transaction:
                    if fee > regular_ssdcoin_to_claim:
                        ssdcoin_tx, _ = await self.create_tandem_ssd_tx(
                            fee,
                            uint64(regular_ssdcoin_to_claim),
                            tx_config,
                    ...
                    elif regular_ssdcoin_to_claim > fee:
                        ssdcoin_tx, _ = await self.create_tandem_ssd_tx(
                            fee,
                            uint64(regular_ssdcoin_to_claim),
                            tx_config,
        ...
        ssdcoin_spend_bundle = SpendBundle([], G2Element())
        if ssdcoin_tx is not None and ssdcoin_tx.spend_bundle is not None:
            ssdcoin_spend_bundle = ssdcoin_tx.spend_bundle

        return (
            SpendBundle.aggregate(
                [
                    cat_spend_bundle,
                    ssdcoin_spend_bundle,
                ]
            ),
            ssdcoin_tx,
        )
    ...
    async def generate_signed_transaction(
        ...
        **kwargs: Unpack[GSTOptionalArgs],
        ...
        if ssdcoin_tx is not None:
            tx_list.append(
                TransactionRecord(
                    confirmed_at_height=ssdcoin_tx.confirmed_at_height,
                    created_at_time=ssdcoin_tx.created_at_time,
                    to_puzzle_hash=ssdcoin_tx.to_puzzle_hash,
                    amount=ssdcoin_tx.amount,
                    fee_amount=ssdcoin_tx.fee_amount,
                    confirmed=ssdcoin_tx.confirmed,
                    sent=ssdcoin_tx.sent,
                    spend_bundle=None,
                    additions=ssdcoin_tx.additions,
                    removals=ssdcoin_tx.removals,
                    wallet_id=ssdcoin_tx.wallet_id,
                    sent_to=ssdcoin_tx.sent_to,
                    trade_id=ssdcoin_tx.trade_id,
                    type=ssdcoin_tx.type,
                    name=ssdcoin_tx.name,
                    memos=[],
                    valid_times=parse_timelock_info(extra_conditions),
                )
            )
