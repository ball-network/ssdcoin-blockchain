...
from ssdcoin.util.db_wrapper import DBWrapper2, execute_fetchone
...
from ssdcoin.wallet.util.query_filter import AmountFilter, FilterMode, HashFilter
    ...
    async def get_coin_records(
        ...
        *,
        ...
        if confirmed_range is not None and confirmed_range != UInt32Range():
            if confirmed_range.start == confirmed_range.stop:
                conditions.append(f"confirmed_height = {confirmed_range.start}")
            else:
        ...
        if spent_range is not None and spent_range != UInt32Range():
            if spent_range.start == spent_range.stop:
                conditions.append(f"spent_height = {spent_range.start}")
            else:
