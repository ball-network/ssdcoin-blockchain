...
from ssdcoin.consensus.blockchain import AddBlockResult
...
from ssdcoin.util.db_wrapper import manage_connection
from ssdcoin.util.errors import KeychainIsEmpty, KeychainIsLocked, KeychainKeyNotFound, KeychainProxyConnectionFailure
from ssdcoin.util.hash import std_hash
from ssdcoin.util.ints import uint16, uint32, uint64, uint128, uint8
from ssdcoin.util.keychain import Keychain
from ssdcoin.util.misc import to_batches
...
from ssdcoin.wallet.puzzles.clawback.metadata import AutoClaimSettings
...
from ssdcoin.wallet.util.new_peak_queue import NewPeakItem, NewPeakQueue, NewPeakQueueTypes
from ssdcoin.wallet.util.peer_request_cache import PeerRequestCache, can_use_peer_request_cache
...
from ssdcoin.wallet.wallet_weight_proof_handler import WalletWeightProofHandler, get_wp_fork_point
...
class WalletNode:
    ...
    def get_cache_for_peer(self, peer: WSSSDCoinConnection) -> PeerRequestCache:
    ...
    async def get_key_for_fingerprint(self, fingerprint: Optional[int]) -> Optional[PrivateKey]:
        try:
            ...
            self.log.warning("No keys present. Create keys with the UI, or with the 'ssd keys' program.")
            return None
    ...
    def initialize_wallet_peers(self) -> None:
        ...
        if self.wallet_peers is None and connect_to_unknown_peers and not testing:
            self.wallet_peers = WalletPeers(
                ...
                self.config.get("dns_servers", ["dns-introducer.ssdcoin.top"]),
    ...
    def on_disconnect(self, peer: WSSSDCoinConnection) -> None:
    ...
    async def on_connect(self, peer: WSSSDCoinConnection) -> None:
        if self._wallet_state_manager is None:
            return None
    ...
    async def state_update_received(self, request: CoinStateUpdate, peer: WSSSDCoinConnection) -> None:
    ...
    def get_full_node_peer(self) -> WSSSDCoinConnection:
        """
        Get a full node, preferring synced & trusted > synced & untrusted > unsynced & trusted > unsynced & untrusted
        """
        full_nodes: List[WSSSDCoinConnection] = self.get_full_node_peers_in_order()
        if len(full_nodes) == 0:
            raise ValueError("No peer connected")
        return full_nodes[0]

    def get_full_node_peers_in_order(self) -> List[WSSSDCoinConnection]:
        """
        ...
        synced_and_trusted: List[WSSSDCoinConnection] = []
        synced: List[WSSSDCoinConnection] = []
        trusted: List[WSSSDCoinConnection] = []
        neither: List[WSSSDCoinConnection] = []
        all_nodes: List[WSSSDCoinConnection] = self.server.get_connections(NodeType.FULL_NODE)
    ...
    async def get_timestamp_for_height_from_peer(self, height: uint32, peer: WSSSDCoinConnection) -> Optional[uint64]:
        """
    ...
    async def new_peak_wallet(self, new_peak: NewPeakWallet, peer: WSSSDCoinConnection) -> None:
        if self._wallet_state_manager is None:
    ...
    async def new_peak_from_trusted(
        self, new_peak_hb: HeaderBlock, latest_timestamp: uint64, peer: WSSSDCoinConnection
    ) -> None:
    ...
    async def new_peak_from_untrusted(self, new_peak_hb: HeaderBlock, peer: WSSSDCoinConnection) -> bool:
        far_behind: bool = (
    ...
    async def long_sync_from_untrusted(self, syncing: bool, new_peak_hb: HeaderBlock, peer: WSSSDCoinConnection) -> None:
    ...
    async def sync_from_untrusted_close_to_peak(self, new_peak_hb: HeaderBlock, peer: WSSSDCoinConnection) -> bool:
        async with self.wallet_state_manager.lock:
    ...
    async def wallet_short_sync_backtrack(self, header_block: HeaderBlock, peer: WSSSDCoinConnection) -> int:
    ...
    async def fetch_and_update_weight_proof(self, peer: WSSSDCoinConnection, peak: HeaderBlock) -> int:
    ...
    async def get_puzzle_hashes_to_subscribe(self) -> List[bytes32]:
        ...
        all_puzzle_hashes.add(self.wallet_state_manager.farm_puzzle_hash)
    ...
    async def validate_block_inclusion(
        self, block: HeaderBlock, peer: WSSSDCoinConnection, peer_request_cache: PeerRequestCache
    ) -> bool:
    ...
    async def get_coin_state(
        self, coin_names: List[bytes32], peer: WSSSDCoinConnection, fork_height: Optional[uint32] = None
    ) -> List[CoinState]:
    ...
    async def fetch_children(
        self, coin_name: bytes32, peer: WSSSDCoinConnection, fork_height: Optional[uint32] = None
    ) -> List[CoinState]:
    ...
    def set_auto_withdraw_stake(self, auto_withdraw_config: AutoWithdrawStakeSettings) -> Dict[str, Any]:
        if auto_withdraw_config.batch_size < 1:
            auto_withdraw_config = dataclasses.replace(auto_withdraw_config, batch_size=uint16(50))
        auto_withdraw_json = auto_withdraw_config.to_json_dict()
        if "auto_withdraw_stake" not in self.config or self.config["auto_withdraw_stake"] != auto_withdraw_json:
            # Update in memory config
            self.config["auto_withdraw_stake"] = auto_withdraw_json
            # Update config file
            with lock_and_load_config(self.root_path, "config.yaml") as config:
                config["wallet"]["auto_withdraw_stake"] = self.config["auto_withdraw_stake"]
                save_config(self.root_path, "config.yaml", config)
        return auto_withdraw_config.to_json_dict()
