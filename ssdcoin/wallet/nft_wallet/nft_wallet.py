...
from ssdcoin.wallet.util.wallet_sync_utils import fetch_coin_spend
...
class NFTWallet:
    ...
    async def coin_added(self, coin: Coin, height: uint32, peer: WSSSDCoinConnection) -> None:
    ...
    async def puzzle_solution_received(self, coin_spend: CoinSpend, peer: WSSSDCoinConnection) -> None:
    ...
    async def mint_from_did(
        ...
        if ssd_coins is None:
            ssd_coins = await self.standard_wallet.select_coins(uint64(total_amount))
        ...
        ssd_spend = await self.standard_wallet.sign_transaction(ssd_spends)
