...
from ssdcoin.simulator.socket import find_available_listen_port
from ssdcoin.simulator.ssl_certs import (
...
from ssdcoin.simulator.start_simulator import async_main as start_simulator_main
from ssdcoin.ssl.create_ssl import create_all_ssl
from ssdcoin.types.blockchain_format.sized_bytes import bytes32
from ssdcoin.util.bech32m import encode_puzzle_hash
from ssdcoin.util.config import create_default_ssdcoin_config, load_config, save_config
from ssdcoin.util.errors import KeychainFingerprintExists
...
def create_config(
    ...
    # create ssdcoin directories
    create_default_ssdcoin_config(ssdcoin_root)
    create_all_ssl(
        ssdcoin_root,
    ...
    # load config
    config = load_config(ssdcoin_root, "config.yaml")
    ...
    config["simulator"]["farming_address"] = encode_puzzle_hash(get_puzzle_hash_from_key(keychain, fingerprint), "tssd")
    config["simulator"]["plot_directory"] = "test-simulator/plots"
    # save config
    save_config(ssdcoin_root, "config.yaml", config)
    return config


async def start_simulator(ssdcoin_root: Path, automated_testing: bool = False) -> AsyncGenerator[FullNodeSimulator, None]:
    sys.argv = [sys.argv[0]]  # clear sys.argv to avoid issues with config.yaml
    started_simulator = await start_simulator_main(True, automated_testing, root_path=ssdcoin_root)
...
async def get_full_ssdcoin_simulator(
    ssdcoin_root: Path,
    ...
    """
    A ssdcoin root Path is required.
    The ssdcoin root Path can be a temporary directory (tempfile.TemporaryDirectory)
    ...
    This test is fully interdependent and can be used without the rest of the ssdcoin test suite.
    ...
    with Lockfile.create(daemon_launch_lock_path(ssdcoin_root)):
        ...
        crt_path = ssdcoin_root / config["daemon_ssl"]["private_crt"]
        key_path = ssdcoin_root / config["daemon_ssl"]["private_key"]
        ca_crt_path = ssdcoin_root / config["private_ssl_ca"]["crt"]
        ca_key_path = ssdcoin_root / config["private_ssl_ca"]["key"]

        ws_server = WebSocketServer(ssdcoin_root, ca_crt_path, ca_key_path, crt_path, key_path)
        async with SignalHandlers.manage() as signal_handlers:
            await ws_server.setup_process_global_state(signal_handlers=signal_handlers)
            async with ws_server.run():
                async for simulator in start_simulator(ssdcoin_root, automated_testing):
                    yield simulator, ssdcoin_root, config, mnemonic, fingerprint, keychain
