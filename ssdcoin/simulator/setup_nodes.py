...
from ssdcoin.daemon.server import WebSocketServer
...
SimulatorsAndWallets = Tuple[List[FullNodeSimulator], List[Tuple[WalletNode, SSDCoinServer]], BlockTools]
...
) -> AsyncGenerator[Tuple[FullNodeAPI, FullNodeAPI, SSDCoinServer, SSDCoinServer, BlockTools], None]:
    """
...
) -> AsyncGenerator[Tuple[List[FullNodeSimulator], List[Tuple[WalletNode, SSDCoinServer]], BlockTools], None]:
    with TempKeyring(populate=True) as keychain1, TempKeyring(populate=True) as keychain2:
...
async def setup_full_system(
    ...
    Tuple[Any, Any, Harvester, Farmer, Any, Service[Timelord, TimelordAPI], object, object, Any, SSDCoinServer], None
]:
    with TempKeyring(populate=True) as keychain1, TempKeyring(populate=True) as keychain2:
...
async def setup_full_system_inner(
    ...
    Tuple[Any, Any, Harvester, Farmer, Any, Service[Timelord, TimelordAPI], object, object, Any, SSDCoinServer],
]:
    if b_tools is None:
