...
git submodule update --init ssdcoin-blockchain-gui

Set-Location -Path ".\ssdcoin-blockchain-gui" -PassThru
...
# Other than `ssdcoin-blockchain-gui/package/gui`, all other packages are no longer necessary after build.
# Since these unused packages make cache unnecessarily fat, unused packages should be removed.
Write-Output "Remove unused @ssdcoin-network packages to make cache slim"
...
# Remove `packages/gui/node_modules/@ssdcoin-network` because it causes an error on later `electron-packager` command
#Remove-Item "@ssdcoin-network" -Recurse -Force
