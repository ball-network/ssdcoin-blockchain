...
LAST_EXIT_CODE=$?
ls -l dist/mac*/ssdcoin.app/Contents/Resources/app.asar
...
if [ "$(arch)" = "arm64" ]; then
  mv dist/"${DMG_NAME}" dist/ssdcoin-"${SSDCOIN_INSTALLER_VERSION}"-arm64.dmg
  DMG_NAME=ssdcoin-${SSDCOIN_INSTALLER_VERSION}-arm64.dmg
fi
...
if [ "$NOTARIZE" == true ]; then
  ...
  notarize-cli --file="$DMG_NAME" --bundle-id top.ssdcoin.blockchain \
...
# xcrun altool --notarize-app -f SSDCoin-0.1.X.dmg --primary-bundle-id top.ssdcoin.blockchain -u username -p password
