...
# Gets the version from the result of "ssdcoin version"
...
def get_ssdcoin_version() -> str:
    version: str = "0.0"
    output = subprocess.run(["ssd", "version"], capture_output=True)
...
def update_version(package_json_path: str):
    ...
    data["version"] = get_ssdcoin_version()
...
if __name__ == "__main__":
    update_version(f"{os.path.dirname(__file__)}/ssdcoin-blockchain-gui/package.json")
    update_version(f"{os.path.dirname(__file__)}/ssdcoin-blockchain-gui/packages/gui/package.json")
