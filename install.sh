...
if [ "$(uname -m)" = "armv7l" ]; then
  ...
  echo "https://github.com/ball-network/ssdcoin-blockchain/wiki/Raspberry-Pi"
  echo "Exiting."
  exit 1
fi
# Get submodules
...
install_python3_and_sqlite3_from_source_with_yum() {
  ...
  cd "$TMP_PATH"
  # Install sqlite>=3.37
  # yum install sqlite-devel brings sqlite3.7 which is not compatible with ssdcoin
  ...
  # yum install python3 brings Python3.6 which is not supported by ssdcoin
  cd ..
...
if [ "$PYTHON_MAJOR_VER" -ne "3" ] || [ "$PYTHON_MINOR_VER" -lt "7" ] || [ "$PYTHON_MINOR_VER" -ge "12" ]; then
  echo "SSDCoin requires Python version >= 3.7 and  < 3.12.0" >&2
...
echo "https://discord.gg/ssdcoin"
echo ""
echo "Try the Quick Start Guide to running ssdcoin-blockchain:"
echo "https://github.com/ball-network/ssdcoin-blockchain/wiki/Quick-Start-Guide"
echo ""
echo "To install the GUI run '. ./activate' then 'sh install-gui.sh'."
echo ""
echo "Type '. ./activate' and then 'ssd init' to begin."
