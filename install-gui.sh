...
if [ -z "$VIRTUAL_ENV" ]; then
  echo "This requires the ssdcoin python virtual environment."
  echo "Execute '. ./activate' before running."
  exit 1
fi

if [ "$(id -u)" = 0 ]; then
  echo "The SSDCoin Blockchain GUI can not be installed or run by the root user."
  exit 1
fi

# Allows overriding the branch or commit to build in ssdcoin-blockchain-gui
SUBMODULE_BRANCH=$1
...
# https://github.com/ball-network/ssdcoin-blockchain/pull/10460#issuecomment-1054492495
...
if [ ! "$CI" ]; then
  ...
  # Set modified output of `ssdcoin version` to version property of GUI's package.json
...
echo ""
echo "SSDCoin  blockchain install-gui.sh completed."
echo ""
