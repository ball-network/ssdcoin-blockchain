...
from ssdcoin.simulator.block_tools import make_unfinished_block
...
async def run_sync_test(
    ...
    with tempfile.TemporaryDirectory() as root_dir:
        ...
        ssdcoin_init(root_path, should_check_keys=False, v1_db=(db_version == 1))
        ...
        full_node.set_server(cast(SSDCoinServer, FakeServer()))
        async with full_node.manage():
            ...
            peer: WSSSDCoinConnection = cast(WSSSDCoinConnection, FakePeer())
...
async def run_sync_checkpoint(
    ...
    ssdcoin_init(root_path, should_check_keys=False, v1_db=False)
    config = load_config(root_path, "config.yaml")
    ...
    async with full_node.manage():
        peer: WSSSDCoinConnection = FakePeer()  # type: ignore[assignment]
